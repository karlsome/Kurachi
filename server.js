...
function broadcastToMachinePlayer(machineId, data) {
  ...
  console.log(
    `📡 Broadcasted player state to ${clients.length} controller client(s) on ${sessionKey}:`,
    data.type || "message",
  );
}

function emitMachinePlayerState(machineId) {
  const state = sanitizeMachinePlayerState(
    getOrCreateMachinePlayerState(machineId),
  );
...
function broadcastToMachine(machineId, data) {
  ...
  clients.forEach((client) => {
    try {
      ...
      console.error(
        `Error sending to client for ${normalizedMachineId}:`,
        error,
      );
    }
  });
  ...
  console.log(
    `📡 Broadcasted to ${clients.length} client(s) on ${normalizedMachineId}:`,
    data,
  );
}
...
function broadcastToFactory(factoryId, data) {
  ...
  console.log(
    `📡 Broadcasted to ${clients.length} factory TV client(s) on ${factoryId}:`,
    data,
  );
}
...
app.get("/api/machine-state/:machineId", (req, res) => {
  ...
  const playerState = sanitizeMachinePlayerState(
    getOrCreateMachinePlayerState(machineId),
  );
...
app.get("/sse/machine-player/:machineId", (req, res) => {
  ...
  const initialState = sanitizeMachinePlayerState(
    getOrCreateMachinePlayerState(machineId),
  );
  res.write(
    `data: ${JSON.stringify({ type: "connected", machineId, timestamp: new Date().toISOString() })}\n\n`,
  );
  res.write(
    `data: ${JSON.stringify({ type: "player-state", ...initialState })}\n\n`,
  );
  ...
  req.on("close", () => {
    ...
    console.log(
      `❌ Machine player SSE client disconnected from ${machineId}. Remaining: ${clients.length}`,
    );
  });
});
...
app.post("/api/machine-player/claim", (req, res) => {
  ...
  const controllerLabel =
    String(req.body?.controllerLabel || "").trim() || "Factory tablet";

  if (!machineId || !controllerId) {
    return res
      .status(400)
      .json({ error: "machineId and controllerId are required" });
  }
...
app.post("/api/machine-player/heartbeat", (req, res) => {
  ...
  if (state.activeControllerId !== controllerId) {
    return res
      .status(409)
      .json({ error: "Controller lock not held by this tablet" });
  }
...
app.post("/api/machine-player/command", (req, res) => {
  ...
  if (!machineId || !controllerId || !command) {
    return res
      .status(400)
      .json({ error: "machineId, controllerId, and command are required" });
  }
...
app.get("/sse/machine/:machineId", (req, res) => {
  ...
  console.log(
    `✅ New SSE connection established for ${machineId}. Total clients: ${machineConnections.get(machineId).length}`,
  );
  ...
  if (machineLastScan.has(machineId)) {
    const lastScan = machineLastScan.get(machineId);
    console.log(
      `📤 Sending last scan data to new client for ${machineId}:`,
      lastScan,
    );
  ...
  req.on("close", () => {
    ...
    console.log(
      `❌ SSE client disconnected from ${machineId}. Remaining: ${clients.length}`,
    );
  });
});
...
app.get("/sse/factory/:factoryId", (req, res) => {
  ...
  console.log(
    `✅ New factory TV connected to ${factoryId}. Total TVs: ${factoryConnections.get(factoryId).length}`,
  );

  // Send initial connection message
  res.write(
    `data: ${JSON.stringify({ type: "connected", factoryId, timestamp: new Date().toISOString() })}\n\n`,
  );
  ...
  req.on("close", () => {
    ...
    console.log(
      `❌ Factory TV disconnected from ${factoryId}. Remaining: ${clients.length}`,
    );
  });
});
...
app.post("/api/broadcast-scan", async (req, res) => {
  ...
  machineIds.forEach((normalizedMachineId) => {
    ...
    if (isClearAction) {
      ...
      console.log(`💾 Stored last scan for ${normalizedMachineId}:`, {
        sebanggo,
        hinban,
      });
    } else {
      console.log(
        `⚠️ Skipping storage for ${normalizedMachineId} - missing sebanggo or hinban`,
      );
    }
  ...
  if (sebanggo && hinban) {
    try {
      ...
      console.log(
        `📝 Tablet logs inserted for ${machineIds.length} machine(s): ${sebanggo} - Scanned kanban (Session: ${sessionID})`,
      );
    } catch (error) {
...
app.get("/api/generate-session-id", async (req, res) => {
  try {
    ...
    if (!背番号 || !設備 || !工場 || !dateParam) {
      return res
        .status(400)
        .json({ error: "背番号, 設備, 工場, and Date are required" });
    }
    ...
    const existingSessions = await tabletLogDB
      .aggregate([
...
app.post("/api/creatomate/renders", async (req, res) => {
  try {
    ...
    if (!CREATOMATE_API_KEY) {
      return res
        .status(500)
        .json({ error: "Creatomate API key not configured" });
    }
    ...
    res
      .status(500)
      .json({ error: "Creatomate render failed", details: error.message });
  }
});
...
app.get("/api/creatomate/renders/:id", async (req, res) => {
  try {
    ...
    const response = await fetch(
      `https://api.creatomate.com/v1/renders/${req.params.id}`,
      {
...
app.post("/api/tablet-log", async (req, res) => {
  try {
    ...
    const isSetupAction = setupActions.some((setupAction) =>
      Action.includes(setupAction),
    );

    if (!背番号 && !isSetupAction) {
      return res
        .status(400)
        .json({ error: "背番号 is required for this action" });
    }
    ...
    if (!sessionID) {
      ...
      return res.status(400).json({
        error:
          "sessionID is required. Frontend must generate and provide sessionID.",
        action: Action,
        設備: 設備,
      });
    }
    ...
    console.log(
      `📝 Tablet log inserted: ${背番号} - ${Action} (Session: ${sessionID})`,
    );
...
app.post("/api/check-existing-pdfs", async (req, res) => {
  try {
    ...
    const existingPDFs = await productPDFsDB
      .find({
        pdfType,
        背番号Array: { $in: 背番号Array },
        isActive: true,
      })
      .project({ 背番号Array: 1, uploadedAt: 1, uploadedBy: 1, fileName: 1 })
      .toArray();
    ...
    res
      .status(500)
      .json({ error: "Error checking existing PDFs", details: error.message });
  }
});
...
app.post("/api/upload-product-pdf", async (req, res) => {
  try {
    ...
    const pdfFile = admin
      .storage()
      .bucket()
      .file(`productPDFs/originals/${pdfType}/${pdfFileName}`);
    ...
    res
      .status(500)
      .json({ error: "Error uploading PDF", details: error.message });
  }
});
...
app.post("/api/upload-pdf-image", async (req, res) => {
  try {
    ...
    const imageFile = admin
      .storage()
      .bucket()
      .file(`productPDFs/images/${pdfType}/${imageFileName}`);
...
app.get("/api/product-pdfs/:sebanggo", async (req, res) => {
  try {
    ...
    const pdfs = await productPDFsDB
      .find({
        背番号Array: sebanggo,
        isActive: true,
      })
    ...
    const result = {
      ...
      ワンポイント確認票:
        pdfs.find((p) => p.pdfType === "ワンポイント確認票") || null,
    ...
    res
      .status(500)
      .json({ error: "Error fetching PDFs", details: error.message });
  }
});
...
app.get("/api/product-pdfs-by-type/:pdfType", async (req, res) => {
  ...
  const _pdfInflightPromise = new Promise((resolve, reject) => {
    _pdfInflightResolve = resolve;
    _pdfInflightReject = reject;
  });
  ...
  try {
    ...
    const needsLookup =
      includeHinban || regexes.length > 0 || Boolean(modelFilter);
    const sortMap = {
      sebanggo: "背番号Array",
...
app.delete("/api/product-pdf/:documentId", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Error deleting PDF", details: error.message });
  }
});
...
app.post("/api/product-pdf-batch-delete", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Error batch deleting PDFs", details: error.message });
  }
});
...
app.get("/api/product-pdfs-trash", async (req, res) => {
  try {
    ...
    const items = await productPDFsDB
      .find(filter)
      .project({
        pdfType: 1,
        背番号Array: 1,
    ...
    res
      .status(500)
      .json({ error: "Error fetching trash", details: error.message });
  }
});
...
app.post("/api/product-pdf-recover/:documentId", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Error recovering PDF", details: error.message });
  }
});
...
app.delete("/api/product-pdf-permanent/:documentId", async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      message: "PDF permanently deleted from all locations",
    });
  } catch (error) {
...
async function cleanupOldTrash() {
  try {
    ...
    const oldItems = await productPDFsDB
      .find({
        isActive: false,
    ...
    console.log(
      `🧹 Found ${oldItems.length} items to permanently delete (>30 days old)`,
    );
    ...
    if (oldItems.length > 0) {
      console.log(
        `✅ Cleanup complete: ${oldItems.length} items permanently deleted`,
      );
    }
  } catch (error) {
...
app.post("/createMasterUser", async (req, res) => {
  ...
  try {
    ...
    if (existsInMaster) {
      return res
        .status(400)
        .json({ error: "Username already exists (master level)" });
    }
    ...
    for (const db of dbs.databases) {
      if (
        ["admin", "local", "config", "Sasaki_Coating_MasterDB"].includes(
          db.name,
        )
      )
        continue;
      const userCol = client.db(db.name).collection("users");
      const existsInCustomer = await userCol.findOne({
        username: normalizedUsername,
      });
      if (existsInCustomer) {
        return res
          .status(400)
          .json({ error: "Username already exists in a customer database" });
      }
    }
    ...
    res
      .status(201)
      .json({ message: "Master user and customer DB created successfully" });
  } catch (err) {
...
if (serviceAccount.private_key && serviceAccount.client_email) {
  ...
  console.log(
    "Firebase Admin SDK initialized successfully using credential path!",
  );
} else {
  console.error(
    "Firebase Admin SDK initialization failed. Ensure either FIREBASE_PRIVATE_KEY and FIREBASE_CLIENT_EMAIL or FIREBASE_CREDENTIAL_PATH are set in your .env file.",
  );
}
...
app.get("/getSetsubiList", async (req, res) => {
  try {
    ...
    console.log(
      `✅ Found ${result.length} unique equipment:`,
      result.map((r) => r.設備).join(", "),
    );
...
app.get("/getActualProductionByEquipment", async (req, res) => {
  try {
    ...
    if (!factory || !date) {
      return res
        .status(400)
        .json({ error: "Factory and date parameters are required" });
    }
    ...
    console.log(
      `📊 Fetching actual production for factory: ${factory}, date: ${date}`,
    );
    ...
    console.log(
      `✅ Found production data for ${result.length} equipment:`,
      result,
    );
...
app.post("/submitTopressDBiReporter", async (req, res) => {
  try {
    ...
    if (materialLabelImages && materialLabelImages.length > 0) {
      console.log(
        `📸 Uploading ${materialLabelImages.length} material label images...`,
      );
      ...
      console.log(
        `✅ Uploaded ${materialLabelImageURLs.length} material label images`,
      );
    }
    ...
    if (maintenanceImages && maintenanceImages.length > 0) {
      console.log(
        `📸 Uploading ${maintenanceImages.length} maintenance images...`,
      );
      ...
      if (formData.Maintenance_Data && formData.Maintenance_Data.records) {
        formData.Maintenance_Data.records =
          formData.Maintenance_Data.records.map((record) => {
      ...
      console.log(
        `✅ Uploaded maintenance images and updated Maintenance_Data structure`,
      );
    }
    ...
    console.log(
      `✅ Successfully saved Press Cutting record with ID: ${result.insertedId}`,
    );
    ...
    res
      .status(500)
      .json({ error: "Error saving data to pressDB", details: error.message });
  }
});
  ...
  try {
    ...
    let updateDoc = {
      ...
      $push: {
        PrintLog: {
          ...printLogEntry,
          timestamp: new Date(printLogEntry.timestamp),
        },
      },
    };
    ...
    if (imagesToUpload && imagesToUpload.length > 0) {
      console.log(
        `🔵 Uploading ${imagesToUpload.length} images to Firebase...`,
      );
      ...
      for (const imgData of imagesToUpload) {
        if (!imgData.base64 || !imgData.label) {
          console.warn(
            "Skipping image due to missing base64 or label",
            imgData.label,
          );
          continue;
        }
        try {
          ...
          const safe品番 = (imgData.品番ForFilename || "unknown品番").replace(
            /[^a-zA-Z0-9-_]/g,
            "_",
          );
          const safe作業日 = (imgData.dateForFilename || "unknownDate").replace(
            /[^a-zA-Z0-9-_]/g,
            "_",
          );
          const safeFactory = (
            imgData.factoryForFilename || "unknownFactory"
          ...
          console.error(
            `Error uploading one image to Firebase (label: ${imgData.label}):`,
            uploadError,
          );
      ...
      if (uploadedImageURLs.length > 0) {
        updateDoc.$addToSet = {
          MaterialLabelImageURLs: { $each: uploadedImageURLs },
        };
      }
    }
    ...
    console.log(
      "🟠 Updating MongoDB document with query:",
      query,
      "and updateDoc:",
    ...
    const updateResult = await collection.updateOne(query, updateDoc, {
      upsert: false,
    }); // Consider upsert:true if the doc might not exist but should be created

    console.log(
      `✅ Main update result: matchedCount: ${updateResult.matchedCount}, modifiedCount: ${updateResult.modifiedCount}, upsertedId: ${updateResult.upsertedId}`,
    );
    ...
    if (updateResult.matchedCount === 0 && !updateResult.upsertedId) {
      ...
      console.warn(
        "Target document not found for update, and upsert was false or failed to match.",
      );
      ...
      return res.status(404).json({
        status: "not_found",
        message:
          "Target document not found for update. No changes made to print counts or logs.",
      });
    }
    ...
    if (updatedDoc) {
      ...
      if (
        targetProductionCountForStatusUpdate > 0 &&
        newTotalPrinted >= targetProductionCountForStatusUpdate
      ) {
        if (updatedDoc.STATUS !== "Completed") {
          // Only update if not already completed
          console.log(
            `🔵 Target production count met (${newTotalPrinted}/${targetProductionCountForStatusUpdate}). Updating STATUS to Completed.`,
          );
          await collection.updateOne(query, {
            $set: { STATUS: "Completed", CompletionTimestamp: new Date() },
          });
          finalStatus = "completed";
        } else {
          finalStatus = "completed"; // Already was completed
          console.log(
            "🔵 Target production count met, but STATUS was already Completed.",
          );
        }
      } else {
      ...
      console.warn(
        "Could not retrieve document after update to check status. This should not happen if update was successful.",
      );
    }
  ...
  try {
    ...
    for (const img of cycleCheckImages) {
      ...
      try {
        ...
        if (img.label === "初物チェック")
          uploadedImageURLs["初物チェック画像"] = publicUrl;
        else if (img.label === "終物チェック")
          uploadedImageURLs["終物チェック画像"] = publicUrl;
        // 材料ラベル is now handled by the new multi-photo system below

        console.log(
          `✅ Cycle check image uploaded: ${img.label} -> ${publicUrl}`,
        );
      } catch (uploadError) {
        console.error(
          `❌ Error uploading cycle check image ${img.label}:`,
          uploadError,
        );
      }
    }
    ...
    for (const imgData of maintenanceImages) {
      ...
      try {
        const buffer = Buffer.from(imgData.base64, "base64");
        console.log(
          `🔍 Processing maintenance image ${imgData.id}: buffer size = ${buffer.length} bytes`,
        );
        ...
        console.error(
          `❌ Error uploading maintenance image ${imgData.id}:`,
          uploadError,
        );
      }
    }
    ...
    if (materialLabelImages.length > 0) {
      console.log(
        `🖼️ Processing ${materialLabelImages.length} material label images...`,
      );
      ...
      for (const imgData of materialLabelImages) {
        ...
        try {
          const buffer = Buffer.from(imgData.base64, "base64");
          console.log(
            `🔍 Processing material label image ${imgData.id}: buffer size = ${buffer.length} bytes`,
          );
          ...
          console.error(
            `❌ Error uploading material label image ${imgData.id}:`,
            uploadError,
          );
        }
      }
    ...
    if (materialLabelImageURLs.length === 1) {
      ...
      uploadedImageURLs["materialLabelImageCount"] =
        materialLabelImageURLs.length;
      console.log(
        `📄 Single material label image: stored in both 材料ラベル画像 field and materialLabelImages array`,
      );
      ...
      uploadedImageURLs["materialLabelImageCount"] =
        materialLabelImageURLs.length; // Count for reference
      console.log(
        `📄 Multiple material label images: ${materialLabelImageURLs.length} images stored`,
      );
    ...
    console.log(
      `🧹 Cleaned pressDBData - removed inspection fields for pressDB-only storage`,
    );
    ...
    if (formData.isToggleChecked) {
      ...
      const Total_NG_Kensa = Object.values(counters).reduce(
      ...
      const _dcpExistingCorrection = await kensaDB.findOne({
        ...
        approvalStatus: {
          $in: ["correction_needed", "correction_needed_from_kacho"],
        },
      });
      ...
      if (_dcpExistingCorrection) {
        ...
        ].forEach((k) => delete _dcpDataFields[k]);
        ...
        console.log(
          `🔄 /submitToDCP: updated existing correction_needed doc ${_dcpExistingCorrection._id} instead of inserting duplicate`,
        );
      } else {
        kensaResult = await kensaDB.insertOne(kensaDBData);
        console.log(
          `✅ Data saved to kensaDB with ID: ${kensaResult.insertedId}`,
        );
      }
...
app.post("/submitToKensaDBiReporter", async (req, res) => {
  try {
    ...
    // Check for correction_needed doc before inserting (prevents duplicate on re-submit)
    const _iRExistingCorrection =
      formData.背番号 && formData.工場 && formData.Date
    ...
    if (_iRExistingCorrection) {
      ...
      [
        ...
        "correctionResponseBy",
        "correctionResponseAt",
      ].forEach((k) => delete _iRDataFields[k]);
        ...
        { _id: _iRExistingCorrection._id },
        {
          ...
          $push: {
            approvalHistory: {
              ...
              timestamp: new Date(),
              comment:
                "⚠️ 修正要求に対してiReporter再提出で修正されました（重複防止）",
            },
          },
        },
      );
      ...
      console.log(
        `🔄 /submitToKensaDBiReporter: updated existing correction_needed doc ${_iRExistingCorrection._id} instead of inserting duplicate`,
      );
...
app.get("/getProductDetails", async (req, res) => {
  try {
    ...
    // Check for duplicates of `背番号`
    let duplicateCount = await masterCollection.countDocuments({
      背番号: serialNumber,
    });
    ...
    if (!productDetails) {
      productDetails = await masterCollection.findOne(
        { 品番: serialNumber },
        {
          projection: {
            品番: 1,
  ...
  try {
    ...
    if (_existingCorrection) {
      ...
      [
        ...
        "correctionResponseBy",
        "correctionResponseAt",
      ].forEach((k) => delete _dataFields[k]);
        ...
        { _id: _existingCorrection._id },
        {
          ...
          $push: {
            approvalHistory: {
              ...
              timestamp: new Date(),
              comment:
                "⚠️ 修正要求に対してタブレスト再提出で修正されました（重複防止）",
            },
          },
        },
      );
      ...
      console.log(
        `🔄 /submitToKensaDB: updated existing correction_needed doc ${_existingCorrection._id} instead of inserting duplicate`,
      );
...
app.post("/saveScannedQRData", async (req, res) => {
  try {
    ...
    const masterDB = client
      .db("Sasaki_Coating_MasterDB")
      .collection("masterDB"); // Collection to fetch 背番号 and 工場
    ...
    if (!scannedBy || !scannedResults || scannedResults.length === 0) {
      return res
        .status(400)
        .json({ error: "No scanned data or name provided" });
    }
  ...
  try {
    ...
    if (query && query._id && typeof query._id === "string") {
      ...
      try {
        ...
        return res
          .status(400)
          .json({ error: "Invalid _id format provided in query." });
      }
    }
    ...
    if (insertData) {
      ...
      const insertResult = Array.isArray(insertData)
        ? await collection.insertMany(insertData)
        : await collection.insertOne(insertData);
      const insertedCount =
        insertResult.insertedCount || (insertResult.insertedId ? 1 : 0);
    ...
    if (deleteFlag) {
      ...
      console.log(
        `✅ Archived ${archivedDocs.length} docs by "${username}" and deleted ${deleteResult.deletedCount} from original.`,
      );
      res.json({
    ...
    if (aggregation) {
      console.log(
        "🔵 Running Aggregation Pipeline with pipeline:",
        aggregation,
      );
    ...
    res
      .status(500)
      .json({ error: "Error executing query", details: error.message });
  }
  // finally {
...
app.post("/api/search-manufacturing-lot", async (req, res) => {
  ...
  try {
    ...
    console.log(
      `🔍 Searching manufacturing lot: "${manufacturingLot}" across ALL factories and dates`,
    );
    ...
    console.log(
      `🔍 Created regex pattern for "${manufacturingLot}":`,
      lotRegex.source,
    );
    ...
    for (const config of collectionsConfig) {
      try {
        ...
        if (config.name === "materialRequestDB") {
          ...
          const orConditions = [{ [config.lotField]: { $regex: lotRegex } }];
        ...
        console.log(
          `🔍 Searching ${config.name} with query:`,
        ...
        if (data && data.length > 0) {
          ...
          console.log(
            `✅ Found ${data.length}/${totalCount} records in ${config.name}`,
          );
        } else {
    ...
    const totalResults = Object.values(results).reduce(
      (sum, processData) => sum + processData.length,
      0,
    );

    console.log(
      `✅ Manufacturing lot search completed. Found ${totalResults} total results across ${Object.keys(results).length} processes.`,
    );
...
app.get("/api/labelComparator/customers", async (req, res) => {
  try {
    await client.connect();
    const database = client.db("Sasaki_Coating_MasterDB");
    const customersCollection = database.collection(
      "labelComparatorCustomerDB",
    );
...
app.post("/api/check-material-sebanggo", async (req, res) => {
  try {
    ...
    console.log(
      `✅ Found ${材料背番号Array.length} 材料背番号 values:`,
      材料背番号Array,
    );
...
app.post("/api/material-lot-lookup", async (req, res) => {
  try {
    ...
    console.log("🔍 Material lot lookup request:", {
      品番,
      材料ロット,
      specified材料背番号,
    });
    ...
    if (specified材料背番号) {
      ...
      if (材料背番号.includes(",")) {
        ...
        const 材料背番号Array = 材料背番号
          .split(",")
          .map((s) => s.trim())
          .filter((s) => s.length > 0);
        console.log(
          `⚠️ Multiple 材料背番号 values found: ${材料背番号Array.join(", ")}`,
        );
    ...
    console.log(
      "🔍 Querying materialRequestDB with:",
      JSON.stringify(query, null, 2),
    );

    const results = await materialCollection
      .find(query)
...
app.post("/api/paginate", async (req, res) => {
  ...
  try {
    ...
    console.log(
      `📄 Pagination request: Page ${currentPage}, Limit ${itemsPerPage}/${maxAllowedLimit} for ${dbName}.${collectionName}`,
    );
    ...
    console.log(
      `✅ Pagination Results: Page ${currentPage}/${totalPages}, ${results.length}/${totalCount} items`,
    );
...
app.post("/api/sensor-history", async (req, res) => {
  ...
  try {
    ...
    console.log(
      `🌡️ Sensor pagination: Device ${deviceId}, Page ${currentPage}, Limit ${itemsPerPage}/${maxAllowedLimit}`,
    );
    ...
    const transformedData = dataResult.map((record) => ({
      ...
      temperature: parseFloat(
        (record.Temperature || "0").toString().replace("°C", "").trim(),
      ),
      humidity: parseFloat(
        (record.Humidity || "0").toString().replace("%", "").trim(),
      ),
    ...
    console.log(
      `✅ Sensor History: Device ${deviceId}, Page ${currentPage}/${totalPages}, ${transformedData.length}/${countResult} records`,
    );
...
app.post("/api/sensor-history/export-csv", async (req, res) => {
  ...
  try {
    ...
    console.log(
      `📥 Exporting sensor data: Device ${deviceId}, Range ${queryStartDate.toISOString().split("T")[0]} to ${queryEndDate.toISOString().split("T")[0]}`,
    );
    ...
    console.log(
      `📤 Sending CSV file: ${filename} (${allRecords.length} records)`,
    );
...
app.post("/api/approval-paginate", async (req, res) => {
  ...
  try {
    ...
    console.log(
      `✅ Approval pagination: ${collectionName}, Page ${currentPage}, Limit ${itemsPerPage}/${maxAllowedLimit}`,
    );
    ...
    if (filters.Date) {
      ...
      const endOfDayLocal = new Date(
        targetDate + "T23:59:59.999" + offsetString,
      );

      // Create ObjectIds at start and end of the day for timestamp-based filtering
      const startObjectId = ObjectId.createFromTime(
        Math.floor(startOfDayLocal.getTime() / 1000),
      );
      const endObjectId = ObjectId.createFromTime(
        Math.floor(endOfDayLocal.getTime() / 1000),
      );

      dateMatchStage._id = { $gte: startObjectId, $lte: endObjectId };
      console.log(
        `📅 Date filter: ${targetDate} (Offset: ${offsetString}) → ObjectId range [${startObjectId}, ${endObjectId}]`,
      );
      console.log(
        `   Start: ${startOfDayLocal.toISOString()} | End: ${endOfDayLocal.toISOString()}`,
      );
    }
    ...
    const dateErrors = dataResult.filter(
      (doc) => doc._hasDateMismatch === 1,
    ).length;
    const timeWarnings = dataResult.filter(
      (doc) => doc._hasTimeMismatch === 1,
    ).length;

    console.log(
      `✅ Approval Pagination: ${collectionName}, Page ${currentPage}/${totalPages}, ${dataResult.length}/${countResult} records`,
    );
    console.log(
      `   🔴 Date errors: ${dateErrors}, ⚠️ Time warnings: ${timeWarnings}, ✅ Sorted by FIFO (oldest first)`,
    );
...
app.post("/api/master-paginate", async (req, res) => {
  ...
  try {
    ...
    console.log(
      `🗂️ Master DB pagination: ${collectionName}, Page ${currentPage}, Limit ${itemsPerPage}/${maxAllowedLimit}`,
    );
    ...
    console.log(
      `✅ Master DB Pagination: Page ${currentPage}/${totalPages}, ${dataResult.length}/${countResult} records`,
    );
...
app.post("/api/approval-stats", async (req, res) => {
  ...
  try {
    ...
    console.log(
      `📊 Computing approval stats for: ${collectionName}, Role: ${userRole}`,
    );
    ...
    if (filters.Date) {
      ...
      console.log(
        `📅 Stats date filter: ${targetDate} (Offset: ${offsetString}) → ObjectId range`,
      );
    }
    ...
    const todayStartObjectId = ObjectId.createFromTime(
      Math.floor(todayStartLocal.getTime() / 1000),
    );
    const todayEndObjectId = ObjectId.createFromTime(
      Math.floor(todayEndLocal.getTime() / 1000),
    );
    ...
    const todayTotal =
      stats.todayStats && stats.todayStats.length > 0
        ? stats.todayStats[0].count
        : 0;

    // Get overall total
    const overallTotal =
      stats.totalCount && stats.totalCount.length > 0
        ? stats.totalCount[0].count
        : 0;

    console.log(
      `✅ Approval Statistics computed: Total: ${overallTotal}, Today: ${todayTotal}`,
    );
...
app.post("/api/approval-factories", async (req, res) => {
  ...
  try {
    ...
    console.log(
      `🏭 Getting factory list for: ${collectionName}, Role: ${userRole}`,
    );
    ...
    const factoryResults = await collection
      .aggregate(factoryAggregation)
      .toArray();
    ...
    console.log(
      `✅ Found ${filteredFactories.length} factories:`,
      filteredFactories,
    );
...
function _invalidateFinancialsCache(reason) {
  ...
  if (n > 0)
    console.log(
      `🗑️  financials cache invalidated (${n} entr${n === 1 ? "y" : "ies"}) — reason: ${reason || "write"}`,
    );
}
...
const _FINANCIALS_SORTERS = {
  ...
  ngAfterRecovery: (a, b) =>
    Math.max((a.totalNg || 0) - (a.recoveredNg || 0), 0) -
    Math.max((b.totalNg || 0) - (b.recoveredNg || 0), 0),
...
app.post("/api/financials", async (req, res) => {
    ...
    if (/^\d{4}-\d{2}-\d{2}$/.test(s)) {
      ...
      if (digits.length >= 8) {
        ...
        if (
          ...
          yr = py;
          mo = pm;
          dy = pd; // true yyyymmdd (e.g. 20251205)
        } else {
  ...
  try {
    ...
    const hinbansArray = Array.isArray(hinbans)
      ? hinbans.filter((h) => h && String(h).trim())
      : [];
    const bansArray = Array.isArray(bans)
      ? bans.filter((b) => b && String(b).trim())
      : [];
    ...
    if (_cached && Date.now() - _cached.ts < _FINANCIALS_CACHE_TTL) {
      ...
      const _paged = _total
        ? _rows.slice((_page - 1) * limitNumber, _page * limitNumber)
        : [];
      console.log(
        `⚡ financials cache HIT (${_total} rows, key age ${Math.round((Date.now() - _cached.ts) / 1000)}s)`,
      );
      return res.json({
        success: true,
        ..._cached.staticData,
    ...
    if (_inflight) {
      ...
      if (_result) {
        ...
        const _paged2 = _total2
          ? _rows2.slice((_page2 - 1) * limitNumber, _page2 * limitNumber)
          : [];
    ...
    if (bansArray.length > 0) {
      const banDocs = await masterCollection
        .find(
          { 背番号: { $in: bansArray.map((b) => String(b).trim()) } },
          { projection: { 品番: 1, _id: 0 } },
        )
        .toArray();
      const hinbansFromBans = Array.from(
        new Set(banDocs.map((doc) => doc.品番).filter(Boolean)),
      );
      ...
      const modelDocs = await masterCollection
        ...
        .toArray();
      allowedHinbans = Array.from(
        new Set(modelDocs.map((doc) => doc.品番).filter(Boolean)),
      );
    }
    ...
    const _prevTo = new Date(
      new Date(fromDate + "T00:00:00Z").getTime() - _msPerDay,
    );
    const _prevFrom = new Date(
      _prevTo.getTime() - (_rangeDays - 1) * _msPerDay,
    );
    ...
    const [
      ...
      submittedDb
        .collection("pressDB")
        .aggregate(buildPressSimplePipeline(trimmedFactory), {
          allowDiskUse: true,
        })
        .toArray(),
      submittedDb
        .collection("kensaDB")
        .aggregate(buildKensaSimplePipeline(trimmedFactory), {
          allowDiskUse: true,
        })
        .toArray(),
      submittedDb
        .collection("slitDB")
        .aggregate(buildSlitNgPipeline(trimmedFactory), { allowDiskUse: true })
        .toArray(),
      submittedDb
        .collection("SRSDB")
        .aggregate(buildSrsNgPipeline(trimmedFactory), { allowDiskUse: true })
        .toArray(),
      submittedDb
        .collection("pressDB")
        .aggregate(buildPrevPressPipeline(), { allowDiskUse: true })
        .toArray(),
      submittedDb
        .collection("kensaDB")
        .aggregate(buildPrevKensaPipeline(), { allowDiskUse: true })
        .toArray(),
      submittedDb
        .collection("slitDB")
        .aggregate(buildPrevSlitPipeline(), { allowDiskUse: true })
        .toArray(),
      submittedDb
        .collection("SRSDB")
        .aggregate(buildPrevSrsPipeline(), { allowDiskUse: true })
        .toArray(),
    ]);
    ...
    const masterDocs = await masterCollection
      .find(
    ...
    const banToMasterMap = new Map(
      masterDocs.filter((d) => d.背番号).map((d) => [d.背番号, d]),
    );

    // --- Previous period summary (for card deltas) ---
    const prevPressHinbanMap = new Map(
      prevPressRows.map((r) => [
        String(r._id),
        { created: r.created || 0, pressNg: r.pressNg || 0 },
      ]),
    );
    const prevKensaHinbanMap = new Map(
      prevKensaRows.map((r) => [String(r._id), r.kensaNg || 0]),
    );
    const prevSlitHinbanMap = new Map(
      prevSlitRows.map((r) => [String(r._id), r.slitNg || 0]),
    );
    const prevSrsHinbanMap = new Map(
      prevSrsRows.map((r) => [String(r._id), r.srsNg || 0]),
    );
    const allPrevHinbans = new Set([
      ...prevPressRows.map((r) => String(r._id)),
      ...prevKensaRows.map((r) => String(r._id)),
      ...prevSlitRows.map((r) => String(r._id)),
      ...prevSrsRows.map((r) => String(r._id)),
    ]);
    ...
    const previousSummary = {
      ...
      defectRate:
        prevTotalCreated > 0
          ? Number(((prevTotalLoss / prevTotalCreated) * 100).toFixed(2))
          : 0,
      yieldPercent:
        prevTotalCreated > 0
          ? Number(
              (
                ((prevTotalCreated - prevTotalLoss) / prevTotalCreated) *
                100
              ).toFixed(2),
            )
          : 0,
      costRecoveryRate:
        prevTotalCost > 0
          ? Number(
              (((prevTotalCost - prevScrapLoss) / prevTotalCost) * 100).toFixed(
                2,
              ),
            )
          : 0,
    ...
    allKeys.forEach((key) => {
      ...
      const pressData = pressMap.get(key) || {
        hinban: keyHinban,
        ban: keyBan,
        factory: keyFactory,
        created: 0,
      ...
      // Factory totals
      const factoryEntry = factoryTotalsMap.get(factory) || {
        created: 0,
        finalGood: 0,
        totalValue: 0,
        scrapLoss: 0,
    ...
    zeroCreatedRows.forEach((zRow) => {
      ...
      if (target) {
        ...
        target.totalNg =
          target.pressNg + target.slitNg + target.srsNg + target.kensaNg;
        target.finalGood = target.created - target.totalNg;
        target.scrapLoss = Number(
          (target.totalNg * target.pricePerPc).toFixed(2),
        );
        target.value = Number((target.cost - target.scrapLoss).toFixed(2));
        target.yieldPercent = Number(
          (target.created > 0
            ? (target.finalGood / target.created) * 100
            : 0
          ).toFixed(2),
        );
      }
    ...
    rows.forEach((r) => {
      const fe = factoryTotalsMap.get(r.factory) || {
        created: 0,
        finalGood: 0,
        totalValue: 0,
        scrapLoss: 0,
    ...
    const _trendMerge = (r, field) => {
      ...
      const e = trendHinbanDayMap.get(k) || {
        date: dateStr,
        hinban,
        created: 0,
        pressNg: 0,
      ...
      if (field === "press") {
        e.created += r.created || 0;
        e.pressNg += r.pressNg || 0;
      }
      if (field === "kensa") {
        e.kensaNg += r.kensaNg || 0;
      }
      if (field === "slit") {
        e.slitNg += r.slitNg || 0;
      }
    ...
    trendHinbanDayMap.forEach((e) => {
      ...
      if (!price) return;
      const entry = trendDayMap.get(e.date) || {
        label: e.date,
        cost: 0,
        scrapLoss: 0,
        created: 0,
    ...
    let trendPoints = Array.from(trendDayMap.values()).sort((a, b) =>
    ...
    if (useWeekly) {
      ...
      trendPoints.forEach((d) => {
        ...
        const startOfWeek1 = new Date(
          jan4.getTime() - ((jan4.getUTCDay() || 7) - 1) * _msPerDay,
        );
        const weekNum = Math.max(
          Math.floor((dt - startOfWeek1) / (_msPerDay * 7)) + 1,
          1,
        );
      ...
      trendPoints = Array.from(weekMap.values()).sort((a, b) =>
        a.label.localeCompare(b.label),
      );
    }
    const trend = {
    ...
    const pagedRows = totalRows
      ? rows.slice(startIndex, startIndex + limitNumber)
      : [];
    ...
    let adjustedSummary = {
      ...
      defectRate:
        mergedTotalCreated > 0
          ? Number(((mergedTotalLoss / mergedTotalCreated) * 100).toFixed(2))
          : 0,
      yieldPercent:
        mergedTotalCreated > 0
          ? Number(((mergedTotalGood / mergedTotalCreated) * 100).toFixed(2))
          : 0,
    };
    try {
      ...
      if (fromDate || toDate) {
        ...
        const createdRange = {};
        if (fromDate) {
          lotRange.$gte = fromDate;
          createdRange.$gte = `${fromDate}T00:00:00.000Z`;
        }
        if (toDate) {
          lotRange.$lte = toDate;
          createdRange.$lte = `${toDate}T23:59:59.999Z`;
        }
        recoveryDateConditions.push({ lotDate: lotRange });
        recoveryDateConditions.push({
          lotDate: { $exists: false },
          createdAt: createdRange,
        });
        recoveryQuery.$or = recoveryDateConditions;
      }
      const allRecoveries = await recoveryCollection
        .find(recoveryQuery)
        .toArray();
      ...
      rows.forEach((row) => {
        row.recoveredNg = recoveryByBan.get(row.ban) || 0;
      });
      ...
      top5 = [...rows]
        .map((r) => {
          ...
          const adjYield =
            r.created > 0
              ? Number(((adjFinalGood / r.created) * 100).toFixed(2))
              : 0;
          return {
            ban: r.ban,
            hinban: r.hinban,
      ...
      if (recoveryByBan.size > 0) {
        ...
        adjustedSummary.defectRate =
          mergedTotalCreated > 0
            ? Number(((adjTotalLoss / mergedTotalCreated) * 100).toFixed(2))
            : 0;
        adjustedSummary.yieldPercent =
          mergedTotalCreated > 0
            ? Number(((adjFinalGood / mergedTotalCreated) * 100).toFixed(2))
            : 0;
      }
      ...
      const prevCreatedRange = {
        $gte: `${prevFromStr}T00:00:00.000Z`,
        $lte: `${prevToStr}T23:59:59.999Z`,
      };
      prevRecoveryQuery.$or = [
        { lotDate: prevLotRange },
        { lotDate: { $exists: false }, createdAt: prevCreatedRange },
      ];
      const prevRecoveries = await recoveryCollection
        .find(prevRecoveryQuery)
        .toArray();
      ...
      if (recoveryByBanPrev.size > 0) {
        ...
        const adjPrevScrapLoss = Math.max(
          previousSummary.scrapLoss - prevRecoveredScrap,
          0,
        );
        const adjPrevTotalLoss = Math.max(
          previousSummary.totalLoss - prevRecoveredLoss,
          0,
        );
        const adjPrevFinalGood =
          previousSummary.totalCreated - adjPrevTotalLoss;
        ...
        previousSummary.defectRate =
          previousSummary.totalCreated > 0
            ? Number(
                (
                  (adjPrevTotalLoss / previousSummary.totalCreated) *
                  100
                ).toFixed(2),
              )
            : 0;
        previousSummary.yieldPercent =
          previousSummary.totalCreated > 0
            ? Number(
                (
                  (adjPrevFinalGood / previousSummary.totalCreated) *
                  100
                ).toFixed(2),
              )
            : 0;
        previousSummary.costRecoveryRate =
          previousSummary.totalValue > 0
            ? Number(
                (
                  ((previousSummary.totalValue - adjPrevScrapLoss) /
      ...
      console.warn(
        "⚠️ Could not fetch recovery data for summary adjustment:",
        recoveryErr.message,
      );
    }
    ...
    const factoryTotalsPayload = {
      ...
      totalValue: factoryTotals.map((item) =>
    ...
    _financialsCache.set(_cacheKey, {
      ts: Date.now(),
      allRows: rows,
    ...
    if (_inflightResolve)
      _inflightResolve({ allRows: rows, staticData: _staticData });
...
app.post("/api/financials/export", async (req, res) => {
  ...
  if (!fromDate || !toDate) {
    return res
      .status(400)
      .json({ success: false, error: "fromDate and toDate are required" });
  }
  ...
  const csvRows = allRows.map((row) => {
    ...
    const yieldPct =
      created > 0 ? Math.round((finalGood / created) * 10000) / 100 : 0;
    return {
      hinban: row.hinban || "",
      ban: row.ban || "",
      model: row.model || "",
...
app.post("/api/analytics-data", async (req, res) => {
  ...
  try {
    ...
    console.log(
      `📊 Computing analytics data for: ${collectionName} from ${fromDate} to ${toDate}, Role: ${userRole}`,
      req.body.factoryFilter
        ? `Factory: ${req.body.factoryFilter}`
        : "All factories",
    );
    ...
    if (req.body.factoryFilter) {
      ...
      baseQuery["工場"] = { $in: factoryAccess };
      console.log(
        `🔐 Applied role-based factory restriction: ${factoryAccess.join(", ")}`,
      );
    }
    ...
    const totalDefectsExpression = (() => {
      switch (collectionName) {
        ...
        default:
          return {
            $add: fields.defectFields.map((field) => ({
              $ifNull: [`$${field}`, 0],
            })),
          };
      }
                ...
                return [
                  {
                    $group: {
                      _id: null,
                      counter1Total: {
                        $sum: { $ifNull: ["$Counters.counter-1", 0] },
                      },
                      counter2Total: {
                        $sum: { $ifNull: ["$Counters.counter-2", 0] },
                      },
                      counter3Total: {
                        $sum: { $ifNull: ["$Counters.counter-3", 0] },
                      },
                      counter4Total: {
                        $sum: { $ifNull: ["$Counters.counter-4", 0] },
                      },
                      counter5Total: {
                        $sum: { $ifNull: ["$Counters.counter-5", 0] },
                      },
                      counter6Total: {
                        $sum: { $ifNull: ["$Counters.counter-6", 0] },
                      },
                      counter7Total: {
                        $sum: { $ifNull: ["$Counters.counter-7", 0] },
                      },
                      counter8Total: {
                        $sum: { $ifNull: ["$Counters.counter-8", 0] },
                      },
                      counter9Total: {
                        $sum: { $ifNull: ["$Counters.counter-9", 0] },
                      },
                      counter10Total: {
                        $sum: { $ifNull: ["$Counters.counter-10", 0] },
                      },
                      counter11Total: {
                        $sum: { $ifNull: ["$Counters.counter-11", 0] },
                      },
                      counter12Total: {
                        $sum: { $ifNull: ["$Counters.counter-12", 0] },
                      },
                    },
                  },
                  {
      ...
      { $match: climateQuery },
      {
        $addFields: {
          ...
          tempValue: {
            $convert: {
              input: {
                $arrayElemAt: [{ $split: ["$Temperature", " "] }, 0],
              },
              to: "double",
    ...
    console.log(
      `📊 Production Summary: ${combinedData.summary?.[0]?.totalProduction || 0} production quantity, ${combinedData.summary?.[0]?.totalDefects || 0} defects`,
    );
    console.log(
      `🌡️ Climate Data: ${climateData.temperatureTrend?.length || 0} temperature readings, ${climateData.humidityTrend?.length || 0} humidity readings`,
    );

    // Debug: Log sample records to understand data structure
    if (combinedData.sampleRecords && combinedData.sampleRecords.length > 0) {
      console.log(
        `🔍 Sample ${collectionName} records:`,
        JSON.stringify(combinedData.sampleRecords, null, 2),
      );
    }

    // Debug: Log production calculation details
    console.log(
      `🧮 Production calculation using ${fields.productionField} field for ${collectionName}`,
    );
    console.log(
      `🧮 Defect calculation using ${collectionName === "SRSDB" ? "SRS_Total_NG" : "Total_NG"} field for ${collectionName}`,
    );
    console.log(
      "📊 Worker field:",
      fields.workerField,
...
app.post("/api/analytics/top-defect-parts", async (req, res) => {
  try {
    ...
    console.log(
      `Fetching top defect parts from ${collectionName}:`,
      matchStage,
    );
    ...
    const factoryPartsArray = Array.from(factoryMap.entries()).map(
      ([factory, topParts]) => ({
        factory,
        topParts,
      }),
    ...
    console.log(
      `Returning top defect parts for ${factoryPartsArray.length} factories`,
    );
...
app.post("/api/analytics/defect-part-details", async (req, res) => {
  try {
    ...
    console.log(
      `Fetching detailed records from ${collectionName}:`,
      matchQuery,
    );

    // Fetch all matching records
    const records = await coll.find(matchQuery).sort({ Date: -1 }).toArray();
  ...
  try {
    ...
    if (response.ok) {
      ...
      res
        ...
        .json({ message: "Failed to send message", error: errorText });
    }
  } catch (error) {
    res
      .status(500)
...
app.get("/chatworkContacts", async (req, res) => {
  ...
  try {
    ...
    if (response.ok) {
      ...
      res
        .status(response.status)
        .json({ message: "Failed to retrieve contacts", error: errorText });
    }
  } catch (error) {
    res
      .status(500)
...
app.post("/chatWorkSend", async (req, res) => {
  ...
  try {
    ...
    const contact = await chatWorkDB.findOne({
    ...
    if (!contact) {
      return res
        .status(404)
        .json({ message: "Account ID not found in database" });
    }
...
app.post("/api/production-goals", async (req, res) => {
  try {
    const { factory, date, 背番号, 品番, 品名, targetQuantity, createdBy } =
      req.body;
    ...
    if (!factory || !date || !targetQuantity || (!背番号 && !品番)) {
      return res.status(400).json({
        success: false,
        error:
          "Missing required fields: factory, date, targetQuantity, and either 背番号 or 品番",
      });
    }
...
app.put("/api/production-goals/:id", async (req, res) => {
  try {
    ...
    const finalTargetQuantity =
      updates.targetQuantity || currentGoal.targetQuantity;
    const finalScheduledQuantity =
      updates.scheduledQuantity || currentGoal.scheduledQuantity || 0;
    const finalRemainingQuantity =
      updates.remainingQuantity !== undefined
        ? updates.remainingQuantity
        : finalTargetQuantity - finalScheduledQuantity;
...
app.post("/api/production-goals/press-history", async (req, res) => {
  try {
    ...
    for (const item of items) {
      ...
      console.log(
        `  Most frequent: ${mostFrequentEquipment} (${maxCount} times)`,
      );
...
app.post("/loginCustomer", async (req, res) => {
  ...
  try {
    ...
    const globalDB = client.db("Sasaki_Coating_MasterDB");
    const masterUser = await globalDB
    ...
    if (masterUser) {
      ...
      if (today > validUntil)
        return res
          .status(403)
          .json({ error: "Account expired. Contact support." });
    ...
    // 2️⃣ Sub-user login (loop all master users)
    const allMasterUsers = await globalDB
      .collection("masterUsers")
      .find({})
      .toArray();
    ...
    for (const mu of allMasterUsers) {
      const customerDB = client.db(mu.dbName);
      const subUser = await customerDB
        .collection("users")
        .findOne({ username });
...
app.post("/createUser", async (req, res) => {
  const { firstName, lastName, email, username, password, role, factory } =
    req.body;
  ...
  if (
    ...
    return res
      .status(400)
      .json({ error: `Factory is required for ${role} users` });
  }
...
app.post("/updateUser", async (req, res) => {
  const { userId, firstName, lastName, email, role, username, factory } =
    req.body; // Added factory to destructuring
  ...
  try {
    ...
    if (role === "班長" || role === "係長") {
      console.log(
        `✅ Factory assignments: ${JSON.stringify(updateFields["工場"])}`,
      );
    }
...
app.post("/verifyLeader", async (req, res) => {
  ...
  try {
    ...
    if (!user) {
      return res
        .status(404)
        .json({ error: "User not found", authorized: false });
    }
    ...
    res
      .status(500)
      .json({ error: "Internal server error during leader verification." });
  }
});
...
app.post("/updateWorker", async (req, res) => {
  ...
  try {
    ...
    if (result.modifiedCount === 0) {
      return res
        .status(404)
        .json({ error: "Worker not found or no changes made" });
    }
...
app.post("/deleteCustomerSubmittedRecords", async (req, res) => {
  try {
    ...
    if (
      ...
      return res
        .status(400)
        .json({ error: "Missing required fields: dbName, recordIds (array)" });
    }

    if (!role || !username) {
      return res
        .status(400)
        .json({ error: "Missing authentication fields: role, username" });
    }
    ...
    if (role !== "masterUser") {
      return res
        .status(403)
        .json({ error: "Access denied. Only masterUser can delete records." });
    }
...
app.post("/saveImageURL", async (req, res) => {
  const { imageUrl, label, factory, machine, worker, date, sebanggo } =
    req.body;
...
app.post("/updateMasterRecord", async (req, res) => {
  ...
  if (!recordId || !updates || !username || !collectionName) {
    // Add collectionName to validation
    return res.status(400).json({ error: "Missing required fields" });
  }
...
app.post("/batchUpdateMasterRecords", async (req, res) => {
  ...
  if (
    ...
    return res
      .status(400)
      .json({ error: "Missing required fields or invalid recordIds array" });
  }
  ...
  try {
    ...
    const oldRecords = await masterColl
...
app.post("/submitToMasterDB", async (req, res) => {
  ...
  if (!data || !username || !collectionName) {
    ...
    return res
      .status(400)
      .json({ error: "Missing data, username, or collectionName" });
  }
...
app.post("/defectDefinitions", async (req, res) => {
  ...
  if (!model || !counters || !username) {
    return res
      .status(400)
      .json({ error: "Missing required fields: model, counters, username" });
  }
  try {
    await client.connect();
    const db = client.db("Sasaki_Coating_MasterDB");
...
app.get("/api/factories/:collection", async (req, res) => {
  try {
    ...
    const uniqueFactories = await targetCollection
      .aggregate([
        {
          $match: {
    ...
    console.log(
      `✅ Found ${uniqueFactories.length} unique factories in ${collection}`,
    );
    ...
    console.error(
      `❌ Error fetching factories from ${req.params.collection}:`,
      error,
    );
    res.status(500).json({
...
app.get("/api/factories", async (req, res) => {
  try {
    ...
    console.error(
      `❌ Error fetching factories from ${req.query.collection}:`,
      error,
    );
    res.status(500).json({
      error: "Failed to fetch factory list",
  ...
  try {
    ...
    if (!collections || !Array.isArray(collections)) {
      return res.status(400).json({
        ...
        usage:
          'POST /api/factories/batch with body: { collections: ["kensaDB", "pressDB"] }',
      });
    }
    ...
    console.log(
      `📋 Fetching unique factory values from multiple collections: ${collections.join(", ")}`,
    );
    ...
    for (const collectionName of collections) {
      try {
        ...
        console.log(
          `✅ Found ${uniqueFactories.length} unique factories in ${collectionName}`,
        );
...
function _invalidateMasterDataCache(collectionName, reason = "") {
  ...
  for (const key of _masterPaginateCache.keys()) {
    if (
      !collectionName ||
      key.includes(`"collectionName":"${collectionName}"`)
    ) {
  ...
  if (n > 0)
    console.log(
      `🗑️  masterData cache invalidated (${collectionName || "all"})${reason ? " — " + reason : ""}: ${n} entries`,
    );
}
...
function _invalidateMasterFiltersCache(reason = "") {
  ...
  if (n > 0)
    console.log(
      `🗑️  masterFilters cache invalidated${reason ? " — " + reason : ""}: ${n} entries`,
    );
}
...
function _invalidateProductPDFsCache(reason = "") {
  ...
  if (n > 0)
    console.log(
      `🗑️  productPDFs cache invalidated${reason ? " — " + reason : ""}: ${n} entries`,
    );
}
// =========================================================
...
app.post("/api/masterdb/data", async (req, res) => {
  ...
  try {
    ...
    console.log(
      `🔍 masterDB data fetch: ${collectionName}, query: ${JSON.stringify(query)}`,
    );
    ...
    console.log(
      `✅ masterDB data cached: ${collectionName} — ${data.length} records (TTL 1 day)`,
    );
    ...
    res
      .status(500)
      .json({ error: "Failed to fetch master data", details: error.message });
  }
});
...
app.post("/api/masterdb/paginate", async (req, res) => {
  ...
  if (_masterPaginateInflight.has(_cacheKey)) {
    try {
      ...
      return res
        .status(500)
        .json({ error: "Failed to fetch paginated data", details: e.message });
    }
  }
  ...
  try {
    ...
    // ── Sort stage ────────────────────────────────────────────────────────────
    const sortStage = sort.column
      ? { [sort.column]: sort.direction }
      : { _id: 1 };

    // ── Parallel queries ──────────────────────────────────────────────────────
    const skip = (page - 1) * limit;
    const [filteredCount, totalCount, withImageCount, pageData] =
      await Promise.all([
    ...
    console.log(
      `✅ masterDB paginate: ${collectionName} p${page}/${Math.ceil(filteredCount / limit)} (${filteredCount} matching)`,
    );
...
app.get("/api/masterdb/schema", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to fetch schema", details: error.message });
  }
});
...
app.post("/api/masterdb/ids", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to fetch IDs", details: error.message });
  }
});
...
app.get("/api/masterdb/factories", async (req, res) => {
  ...
  try {
    ...
    const uniqueFactories = await collection
      .aggregate([
        {
          $match: {
            工場: { $exists: true, $ne: null, $ne: "" },
    ...
    console.log(
      `✅ Found ${uniqueFactories.length} unique factories in Master DB`,
    );
    ...
    _masterFiltersCache.set("masterdb:factories", {
      ts: Date.now(),
      data: _result,
    });
    res.json(_result);
...
app.get("/api/masterdb/rl", async (req, res) => {
  ...
  try {
    ...
    const uniqueRL = await collection
      .aggregate([
        {
          $match: {
...
app.get("/api/masterdb/colors", async (req, res) => {
  ...
  try {
    ...
    const uniqueColors = await collection
      .aggregate([
        {
          $match: {
    ...
    _masterFiltersCache.set("masterdb:colors", {
      ts: Date.now(),
      data: _result,
    });
    res.json(_result);
...
app.get("/api/masterdb/equipment", async (req, res) => {
  ...
  try {
    ...
    const uniqueEquipment = await collection
      .aggregate([
        {
          $match: {
    ...
    console.log(
      `✅ Found ${uniqueEquipment.length} unique equipment values in Master DB`,
    );
    ...
    _masterFiltersCache.set("masterdb:equipment", {
      ts: Date.now(),
      data: _result,
    });
    res.json(_result);
...
app.get("/api/masterdb/models", async (req, res) => {
  ...
  try {
    ...
    _masterFiltersCache.set("masterdb:models", {
      ts: Date.now(),
      data: _result,
    });
    res.json(_result);
...
app.get("/api/masterdb/products", async (req, res) => {
  try {
    ...
    console.log(
      `✅ Found ${products.length} products for model: ${model || "all"}`,
    );
...
app.get("/api/masterdb/shapes", async (req, res) => {
  ...
  try {
    ...
    const uniqueShapes = await collection
      .aggregate([
        {
          $match: {
    ...
    _masterFiltersCache.set("masterdb:shapes", {
      ts: Date.now(),
      data: _result,
    });
    res.json(_result);
...
app.get("/api/masterdb/materials", async (req, res) => {
  ...
  try {
    ...
    const uniqueMaterials = await collection
      .aggregate([
        {
          $match: {
    ...
    console.log(
      `✅ Found ${uniqueMaterials.length} unique materials in Master DB`,
    );
    ...
    _masterFiltersCache.set("masterdb:materials", {
      ts: Date.now(),
      data: _result,
    });
    res.json(_result);
...
app.get("/api/masterdb/filters", async (req, res) => {
  ...
  const _batchPromise = new Promise((resolve, reject) => {
  ...
  try {
    ...
    for (const { field, key } of fields) {
      try {
        const uniqueValues = await collection
          .aggregate([
            {
              $match: {
    ...
    switch (action) {
      case "getNodaRequests":
        try {
          ...
          if (allBackNumbers.length > 0) {
            const batchInventoryResults = await inventoryCollection
              .aggregate([
            ...
            for (const result of batchInventoryResults) {
              ...
              inventoryMap.set(result._id, {
                physicalQuantity:
                  inv.physicalQuantity || inv.runningQuantity || 0,
                reservedQuantity: inv.reservedQuantity || 0,
                availableQuantity:
                  inv.availableQuantity || inv.runningQuantity || 0,
              });
            }
          ...
          for (const request of allActiveRequests) {
            ...
            for (const lineItem of lineItems) {
              ...
              remainingPhysical.set(
                backNumber,
                Math.max(0, remaining - canPick),
              );
          ...
          const enrichedRequests = requests.map((request) => {
            ...
            const isPastDeadline =
              requestDeadline && requestDeadline < todayStr;
            ...
            return {
              ...
              dynamicInventoryStatus:
                request.overallInventoryStatus || "unknown",
            };
          });
          ...
          const statistics = await calculateNodaStatistics(
            requestsCollection,
            statsQuery,
          );
      ...
      case "getRequestById":
        try {
          ...
          const allSebanForFifo = [
            ...new Set(
              activeRequestsForFifo
                .flatMap((r) =>
                  r.lineItems ? r.lineItems.map((li) => li.背番号) : [r.背番号],
                )
                .filter(Boolean),
          ...
          if (allSebanForFifo.length > 0) {
            const invResults = await inventoryCollection
              .aggregate([
          ...
          for (const activeReq of activeRequestsForFifo) {
            ...
            const lineItems = activeReq.lineItems || [
              {
                背番号: activeReq.背番号,
            ...
            if (isCurrentRequest && request.lineItems) {
              ...
              enrichedLineItems = request.lineItems.map((lineItem) => {
                ...
                remainingPhysicalForFifo.set(
                  backNumber,
                  Math.max(0, remaining - canReserve),
                );
          ...
          if (!enrichedLineItems && request.lineItems) {
            ...
            enrichedLineItems = await Promise.all(
              request.lineItems.map(async (lineItem) => {
                ...
                const invResult = await inventoryCollection
                  .aggregate([
                ...
                const physicalQty =
                  invResult.length > 0
                    ? invResult[0].physicalQuantity ||
                      invResult[0].runningQuantity ||
                      0
                    : 0;
                const needed = lineItem.quantity || 0;
      ...
      case "createRequest":
        try {
          ...
          if (!inventoryItem) {
            return res
              .status(400)
              .json({ error: "Item not found in inventory" });
          }

          // Check available quantity (not physical quantity)
          const availableQuantity =
            inventoryItem.availableQuantity ||
            inventoryItem.runningQuantity ||
            0;
          ...
          const currentPhysical =
            inventoryItem.physicalQuantity ||
            inventoryItem.runningQuantity ||
            0;
          ...
          const newAvailableQuantity =
            currentAvailable - parseInt(data.quantity);
          ...
          if (existingRequest) {
            res.json({
              ...
              request: {
                ...
                totalItems:
                  existingRequest.totalItems ||
                  (existingRequest.lineItems
                    ? existingRequest.lineItems.length
                    : 0),
      ...
      case "bulkCreateRequests":
        try {
          ...
          if (!data.pickupDate) {
            return res
              .status(400)
              .json({ error: "Pickup date is required for bulk request" });
          }
          ...
          if (mode === "overwrite" && existingRequestId) {
            try {
              ...
              if (existingRequest) {
                ...
                if (
                  ...
                  for (const lineItem of existingRequest.lineItems) {
                    ...
                    if (latestInventory.length > 0) {
                      ...
                      const currentPhysical =
                        currentInventory.physicalQuantity ||
                        currentInventory.runningQuantity ||
                        0;
                      ...
                      const newReservedQuantity = Math.max(
                        0,
                        currentReserved - unreserveAmount,
                      );
                      // Only add back to available what was actually reserved from it
                      const actualReservedFromAvailable =
                        lineItem.reservedQuantity ||
                        Math.min(currentReserved, unreserveAmount);
                      const newAvailableQuantity =
                        currentAvailable + actualReservedFromAvailable;

                      console.log(
                        `  📤 ${lineItem.背番号}: Unreserving ${unreserveAmount} units (Reserved: ${currentReserved} → ${newReservedQuantity}, Available: ${currentAvailable} → ${newAvailableQuantity})`,
                      );
                ...
                console.log(
                  `✅ Deleted existing request ${existingRequestId} for overwrite`,
                );
              }
            } catch (error) {
          ...
          for (const item of data.items) {
            try {
              ...
              const inventoryResults = await inventoryCollection
                .aggregate([
                  { $match: { 背番号: item.背番号 } },
                  {
                    $addFields: {
              ...
              if (inventoryResults.length === 0) {
                ...
                console.log(
                  `⚠️ ${item.背番号} not found in inventory - adding with 0 availability`,
                );
                validItems.push({
                  ...item,
                  inventoryItem: null,
              ...
              console.log(
                `⚠️ Error checking inventory for ${item.背番号}: ${error.message} - Adding with 0 inventory`,
              );
              validItems.push({
                ...item,
                inventoryItem: null,
          ...
          if (mode === "overwrite" && oldRequestNumber) {
            ...
            if (mode === "createNew") {
              ...
              const existingRequests = await requestsCollection
                .find({
                  requestNumber: {
                    $regex: `^${baseRequestNumber.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}`,
                  },
                })
                .toArray();

              console.log(
                `Found ${existingRequests.length} existing requests with base: ${baseRequestNumber}`,
              );
              ...
              console.log(
                `Created new request number with suffix: ${bulkRequestNumber}`,
              );
            }
          } else {
            // Fallback to old format: NODAPO-YYYYMMDD-001
            const deadlineDate =
              data.deadlineDate || new Date().toISOString().split("T")[0];
          ...
          const hasNoInventory = validItems.every(
            (item) => item.inventoryStatus === "none",
          );
          const hasPartialInventory = validItems.some(
            (item) =>
              item.inventoryStatus === "insufficient" ||
              item.inventoryStatus === "none",
          );
          const hasSufficientInventory = validItems.every(
            (item) => item.inventoryStatus === "sufficient",
          );
          ...
          for (const item of validItems) {
            const currentPhysical = item.inventoryItem
              ? item.inventoryItem.physicalQuantity ||
                item.inventoryItem.runningQuantity ||
                0
              : 0;
            const currentReserved = item.inventoryItem
              ? item.inventoryItem.reservedQuantity || 0
              : 0;
            ...
            const newAvailableQuantity = Math.max(
              0,
              currentAvailable - item.reservedQuantity,
            ); // Only deduct what's actually available
            ...
            if (item.shortfallQuantity > 0) {
              console.log(
                `⏳ ${item.背番号}: Reserved ${item.reservedQuantity} available, waiting for ${item.shortfallQuantity} more units`,
              );
            }
          }
      ...
      case "addItemsToRequest":
        try {
          if (
            ...
            return res
              .status(400)
              .json({ error: "Request ID and items are required" });
          }
          ...
          if (existingRequest.requestType !== "bulk") {
            return res
              .status(400)
              .json({ error: "Can only add items to bulk requests" });
          }
          ...
          for (const item of data.items) {
            try {
              ...
              const existingLineItem = existingRequest.lineItems.find(
                (lineItem) => lineItem.背番号 === item.背番号,
              );
              if (existingLineItem) {
                failedItems.push({
                  背番号: item.背番号,
          ...
          const currentMaxLineNumber = Math.max(
            ...existingRequest.lineItems.map((item) => item.lineNumber),
          );
      ...
      case "updateRequest":
        try {
          if (!requestId || !data) {
            return res
              .status(400)
              .json({ error: "Request ID and data are required" });
          }
        ...
        try {
          ...
          if (oldStatus !== newStatus) {
            ...
            if (inventoryItem) {
              ...
              if (
                newStatus === "complete" &&
                (oldStatus === "pending" || oldStatus === "active")
              ) {
      ...
      case "updateLineItemStatus":
        try {
          ...
          const bulkRequest = await requestsCollection.findOne({
            _id: new ObjectId(requestId),
          });
          if (!bulkRequest) {
          ...
          if (bulkRequest.requestType !== "bulk") {
            return res
              .status(400)
              .json({ error: "This operation is only for bulk requests" });
          }

          // Find the current line item
          const currentLineItem = bulkRequest.lineItems.find(
            (item) => item.lineNumber === data.lineNumber,
          );
          if (!currentLineItem) {
          ...
          if (currentStatus === "in-progress" && newStatus === "completed") {
            return res.status(400).json({
              error:
                "Cannot change status from 'in-progress' to 'completed' via admin interface. Only ESP32/IoT device can complete in-progress items to prevent inventory mismatches.",
            });
          }
          ...
          if (
            ...
            console.log(
              `🔄 Reversing inventory transaction for line item ${data.lineNumber}: ${currentStatus} → ${newStatus}`,
            );
            ...
            if (inventoryResults.length > 0) {
              ...
              const newPhysicalQuantity =
                (currentInventory.physicalQuantity || 0) + quantity;
              const newReservedQuantity =
                (currentInventory.reservedQuantity || 0) + quantity;
              const newAvailableQuantity =
                currentInventory.availableQuantity || 0; // Available stays the same
              ...
              console.log(
                `✅ Inventory transaction reversed for ${currentLineItem.背番号}: +${quantity} units`,
              );
            } else {
              console.warn(
                `⚠️ No inventory record found for ${currentLineItem.背番号}`,
              );
            }
          }
          ...
          const allCompleted = updatedRequest.lineItems.every(
            (item) => item.status === "completed",
          );
          const anyInProgress = updatedRequest.lineItems.some(
          ...
          if (Object.keys(bulkUpdateFields).length > 1) {
      ...
      case "updateLineItemQuantity":
        try {
          ...
          // Find the line item
          const lineItem = bulkRequest.lineItems.find(
            (item) => item.lineNumber === data.lineNumber,
          );
          if (!lineItem) {
            return res.status(404).json({ error: "Line item not found" });
          ...
          console.log(
            `📦 Updating line item quantity: ${originalQuantity} → ${newQuantity} (diff: ${quantityDiff})`,
          );
          console.log(
            `   Inventory: Reserved ${currentReserved} → ${newReservedQuantity}, Available ${currentAvailable} → ${newAvailableQuantity}`,
          );
          ...
          if (updateResult.modifiedCount === 0) {
            return res
              .status(500)
              .json({ error: "Failed to update line item quantity" });
          }
      ...
      case "deleteLineItem":
        try {
          ...
          if (bulkRequest.lineItems.length <= 1) {
            return res.status(400).json({
              error:
                "Cannot delete the last line item. Please delete the entire request instead.",
            });
          }
          ...
          if (inventoryResults.length > 0) {
            ...
            const newReservedQuantity = Math.max(
              0,
              currentReserved - data.quantity,
            );
            const newAvailableQuantity = currentAvailable + data.quantity;

            console.log(
              `🗑️ Deleting line item ${data.lineNumber}, unreserving ${data.quantity} units`,
            );
            console.log(
              `   Inventory: Reserved ${currentReserved} → ${newReservedQuantity}, Available ${currentAvailable} → ${newAvailableQuantity}`,
            );
          ...
          if (updateResult.modifiedCount === 0) {
            return res
              .status(500)
              .json({ error: "Failed to delete line item" });
          }
      ...
      case "deleteRequest":
        try {
          ...
          if (isBulkRequest && request.lineItems) {
            ...
            console.log(
              `🗑️ Deleting bulk request ${request.requestNumber} with ${request.lineItems.length} line items`,
            );
            ...
            for (const lineItem of request.lineItems) {
              try {
                ...
                if (lineItem.status === "completed") {
                  console.log(
                    `⏭️ Skipping line ${lineItem.lineNumber} (${lineItem.背番号}): Already completed (physically picked)`,
                  );
                ...
                console.log(
                  `🔄 Restoring line ${lineItem.lineNumber} (${lineItem.背番号}): Status = ${lineItem.status}`,
                );
                ...
                if (inventoryResults.length > 0) {
                  ...
                  const newReservedQuantity = Math.max(
                    0,
                    currentReserved - lineItem.quantity,
                  );
                  const newAvailableQuantity =
                    currentAvailable + lineItem.quantity;
                  ...
                  console.log(
                    `✅ Restored ${lineItem.quantity} units for ${lineItem.背番号} (line ${lineItem.lineNumber})`,
                  );
                } else {
                  console.warn(
                    `⚠️ No inventory found for ${lineItem.背番号} (line ${lineItem.lineNumber})`,
                  );
                }
              } catch (error) {
                console.error(
                  `❌ Error restoring inventory for line item ${lineItem.lineNumber} (${lineItem.背番号}):`,
                  error,
                );
              }
            }
            ...
            if (
              ...
              if (inventoryResults.length > 0) {
                ...
                const newReservedQuantity = Math.max(
                  0,
                  currentReserved - request.quantity,
                );
                const newAvailableQuantity =
                  currentAvailable + request.quantity;
                ...
                console.log(
                  `✅ Restored ${request.quantity} units for ${request.背番号}`,
                );
              }
            } else {
              console.log(
                `⏭️ Skipping restoration: Single request status is '${request.status}' (completed requests are not restored)`,
              );
            }
          }
      ...
      case "autoCheckInventory":
        try {
          ...
          const requestsNeedingInventory = await requestsCollection
            .find({
              $or: [
                { overallInventoryStatus: "waiting-for-inventory" },
                { overallInventoryStatus: "partial-inventory" },
            ...
            .sort({ createdAt: 1 })
            .toArray(); // First-come-first-served

          console.log(
            `📋 Found ${requestsNeedingInventory.length} requests needing inventory check`,
          );
          ...
          for (const request of requestsNeedingInventory) {
              ...
              if (lineItem.shortfallQuantity > 0) {
                ...
                if (inventoryResults.length > 0) {
                  ...
                  if (availableQty > 0) {
                    ...
                    console.log(
                      `  ✅ ${lineItem.背番号}: Can reserve ${canReserve}/${lineItem.shortfallQuantity} units (Available: ${availableQty})`,
                    );
                    ...
                    const newReservedQty =
                      lineItem.reservedQuantity + canReserve;
                    const newShortfallQty =
                      lineItem.shortfallQuantity - canReserve;
            ...
            if (requestUpdated) {
              ...
              const hasNoInventory = updatedRequest.lineItems.every(
                (item) => item.inventoryStatus === "none",
              );
              const hasPartialInventory = updatedRequest.lineItems.some(
                (item) =>
                  item.inventoryStatus === "insufficient" ||
                  item.inventoryStatus === "none",
              );
              ...
              console.log(
                `  📦 Updated ${request.requestNumber}: ${requestReservations} line items reserved, status: ${newOverallStatus}`,
              );
            }
          }

          console.log(
            `✅ Auto-check complete: ${totalReservations} reservations across ${updatedRequests} requests`,
          );
      ...
      case "getFilterOptions":
        try {
          ...
          const partNumbersResult = await requestsCollection
            .aggregate([
              { $group: { _id: "$品番" } },
          ...
          const backNumbersResult = await requestsCollection
            .aggregate([
              { $group: { _id: "$背番号" } },
              { $match: { _id: { $ne: null, $ne: "" } } },
              { $sort: { _id: 1 } },
      ...
      case "checkInventory":
        try {
          ...
          console.log(
            `📊 CheckInventory for ${背番号}: Physical=${inventoryInfo.physicalQuantity}, Reserved=${inventoryInfo.reservedQuantity}, Available=${inventoryInfo.availableQuantity}`,
          );
      ...
      case "lookupMasterData":
        try {
          ...
          if (!品番 && !背番号) {
            return res
              .status(400)
              .json({ error: "Either 品番 or 背番号 is required" });
          }
    ...
    switch (action) {
      case "getInventoryData":
        try {
          ...
          if (
            filters.sebanggoArray &&
            Array.isArray(filters.sebanggoArray) &&
            filters.sebanggoArray.length > 0
          ) {
          ...
          if (filters.search) {
            const searchRegex = new RegExp(filters.search, "i");
            matchStage.$or = [{ 品番: searchRegex }, { 背番号: searchRegex }];
          }
          ...
          const formattedItems = paginatedItems.map((item) => ({
            ...
            工場: item.工場,
            physicalQuantity:
              item.physicalQuantity || item.runningQuantity || 0,
            reservedQuantity: item.reservedQuantity || 0,
            availableQuantity:
              item.availableQuantity || item.runningQuantity || 0,
      ...
      case "getFilterOptions":
        try {
          ...
          const partNumbers = [
            ...new Set(
              latestInventory.map((item) => item.品番).filter(Boolean),
            ),
          ].sort();
          const backNumbers = [
            ...new Set(
              latestInventory.map((item) => item.背番号).filter(Boolean),
            ),
          ].sort();
      ...
      case "resetInventory":
        try {
          ...
          if (!backNumber || !partNumber) {
            return res
              .status(400)
              .json({ error: "背番号 and 品番 are required" });
          }
          ...
          console.log(
            `✅ Admin reset: ${backNumber} - ${actionString} by ${submittedBy}`,
          );
      ...
      case "getBatchResetItems":
        try {
          ...
          const modelFilters =
            filters && filters.filter((f) => f.field === "モデル");
          const nonModelFilters =
            filters && filters.filter((f) => f.field !== "モデル");
          ...
          console.log(
            `📋 Found ${items.length} inventory items after initial query`,
          );
          ...
          if (modelFilters && modelFilters.length > 0) {
            ...
            console.log(
              `📝 Enriching ${items.length} items with モデル data from masterDB`,
            );
            ...
            console.log(
              `📝 Sample inventory item after enrichment:`,
              items[0]
                ? {
                    背番号: items[0].背番号,
                    品番: items[0].品番,
              ...
              items = items.filter((item) => {
                if (modelFilter.operator === "equals") {
                  ...
                  return (
                    ...
                    item.モデル
                      .toLowerCase()
                      .includes(modelFilter.value.toLowerCase())
                  );
                }
                return true;
              });
            ...
            console.log(
              `📋 After モデル filtering: ${items.length} items remaining`,
            );
          }
      ...
      case "batchResetInventory":
        try {
          ...
          console.log(
            `✅ Batch reset completed: ${successCount}/${items.length} items reset (Batch ID: ${batchResetId})`,
          );
  ...
  try {
    ...
    if (
      ...
      return res.status(400).json({
        success: false,
        message:
          "Missing required fields: 品番, 背番号, and physicalQuantityChange (must be > 0)",
      });
    }
    ...
    console.log(
      `🔍 [ADD INVENTORY] Looking up latest inventory for 背番号: ${背番号}, 工場: ${工場}`,
    );
    ...
    const latestRecords = await inventoryCollection
      .aggregate([
        { $match: query },
        {
          $addFields: {
    ...
    console.log(
      `📊 [ADD INVENTORY] Current inventory found:`,
      currentInventory
        ? {
    ...
    const currentPhysicalQuantity = currentInventory
      ? currentInventory.physicalQuantity ||
        currentInventory.runningQuantity ||
        0
      : 0;
    const currentReservedQuantity = currentInventory
      ? currentInventory.reservedQuantity || 0
      : 0;

    console.log(
      `📊 [ADD INVENTORY] Current quantities - Physical: ${currentPhysicalQuantity}, Reserved: ${currentReservedQuantity}`,
    );
    console.log(`➕ [ADD INVENTORY] Adding ${physicalQuantityChange} units`);

    // Calculate new quantities
    const newPhysicalQuantity =
      currentPhysicalQuantity + physicalQuantityChange;
    const newAvailableQuantity = newPhysicalQuantity - currentReservedQuantity;

    console.log(
      `📊 [ADD INVENTORY] New quantities - Physical: ${newPhysicalQuantity}, Available: ${newAvailableQuantity}, LastQuantity: ${currentPhysicalQuantity}`,
    );
...
app.post("/api/distinct", async (req, res) => {
  try {
    ...
    console.log(
      `📋 Fetching distinct values for ${field} from ${collectionName}...`,
    );
    ...
    if (!dbName || !collectionName || !field) {
      return res.status(400).json({
        error:
          "Missing required parameters: dbName, collectionName, and field are required",
      });
    }
...
app.get("/api/distinct/test", async (req, res) => {
  try {
    ...
    const workers = await collection.distinct("Worker_Name", {
      工場: "第二工場",
    });
...
app.get("/api/workorders/assignees", async (req, res) => {
  try {
    ...
    const assignees = await collection
      .aggregate([
        {
          $match: {
...
app.put("/api/workorders/:id", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: "Error updating work order", details: error.message });
  }
});
...
app.post("/api/workorders/create", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: "Error creating work order", details: error.message });
  }
});
...
app.delete("/api/workorders/:id", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: "Error deleting work order", details: error.message });
  }
});
...
app.post("/api/workorders/statistics", async (req, res) => {
  ...
  try {
    ...
    const statistics = await calculateWorkOrderStatistics(
      collection,
      baseQuery,
    );
    ...
    res
      .status(500)
      .json({ error: "Error getting statistics", details: error.message });
  }
});
...
app.get("/api/freya-tablet-data", async (req, res) => {
  try {
    ...
    console.log(
      `📊 Pagination calculation: ${totalItems} total items, ${limit} per page = ${totalPages} pages, skipping ${skip} items`,
    );
    ...
    console.log(
      `Found ${records.length} production records for SCNA factory (page ${page}/${totalPages})`,
    );
...
console.log(
  "🏭 Freya Tablet routes with server-side pagination loaded successfully",
);
...
app.post("/api/workorders/bulk-upload", async (req, res) => {
  ...
  try {
    ...
    const existingOrders = await collection
      .find({ Number: { $in: workOrderNumbers } })
      .toArray();

    const existingNumbers = new Set(
      existingOrders.map((order) => order.Number),
    );
    ...
    if (!overwrite && duplicates.length > 0) {
      console.log(
        `📋 Found ${duplicates.length} duplicate work orders, processing ${newOrders.length} new orders`,
      );
    }
    ...
    for (const orderData of ordersToProcess) {
      try {
        ...
        const workOrderWithMetadata = {
          ...
          "Date and time":
            orderData["Date and time"] || currentTime.toISOString(),
          Deadline: orderData["Deadline"]
            ? formatDateToYYYYMMDD(orderData["Deadline"])
            : null,
          ...
          "Material loading (%)":
            Number(orderData["Material loading (%)"]) || 0,
          "Finished goods note (%)":
            Number(orderData["Finished goods note (%)"]) || 0,
        ...
        if (overwrite && existingNumbers.has(orderData.Number)) {
          ...
          const existingOrder = existingOrders.find(
            (order) => order.Number === orderData.Number,
          );
        ...
        console.error(
          `Error processing work order ${orderData.Number}:`,
          orderError,
        );
        errors.push({
          workOrderNumber: orderData.Number || "Unknown",
...
app.post("/api/scna/machine-analytics", async (req, res) => {
  ...
  try {
    ...
    console.log("📊 Fetching SCNA machine analytics...", {
      dateFrom,
      dateTo,
      machine,
    });
    ...
    const machineData = await collection
      .find(matchCriteria)
      .sort({ Date: 1, Time_start: 1 })
      .toArray();
    ...
    const summary = {
      ...
      machines: [...new Set(machineData.map((item) => item.設備))].filter(
        Boolean,
      ),
      dateRange: {
        from: dateFrom,
      ...
      totalWorkHours: machineData.reduce(
        (sum, item) => sum + (item.Total_Work_Hours || 0),
        0,
      ),
      totalBreakHours: machineData.reduce(
        (sum, item) => sum + (item.Total_Break_Hours || 0),
        0,
      ),
      totalProduction: machineData.reduce(
...
app.get("/api/equipment/list", async (req, res) => {
  try {
    ...
    console.log(
      `✅ Found ${allEquipment.length} unique equipment across ${results.length} factories`,
    );
...
function _invalidateFactoryOverviewCache(reason) {
  ...
  if (ns > 0)
    console.log(
      `🗑️  factory stats cache invalidated (${ns} entr${ns === 1 ? "y" : "ies"}) — ${reason || "write"}`,
    );
}
// ─────────────────────────────────────────────────────────────────────────────
...
app.get("/api/factory-overview/stats", async (req, res) => {
  ...
  try {
    ...
    if (_sc && Date.now() - _sc.ts < _FACTORY_OVERVIEW_TTL) {
      console.log(
        `⚡ factory stats cache HIT (date: ${date}, age: ${Math.round((Date.now() - _sc.ts) / 1000)}s)`,
      );
    ...
    if (_si) {
      ...
      if (_r) {
        console.log(`⚡ factory stats stampede WAIT resolved`);
        return res.json(_r);
      }
    }
    const _statsInflightPromise = new Promise((res, rej) => {
      _statsInflightResolve = res;
      _statsInflightReject = rej;
    });
          ...
          $project: {
            ...
            defectRate: {
              $cond: {
                ...
                then: {
                  $round: [
                    { $multiply: [{ $divide: ["$totalNG", "$total"] }, 100] },
                    2,
                  ],
                },
                else: 0,
    ...
    console.log(
      `✅ Factory stats loaded for ${Object.keys(factoryStats).length} factories`,
    );

    const _statsPayload = { success: true, date: date, data: factoryStats };
    _factoryStatsCache.set(_statsCacheKey, {
      ts: Date.now(),
...
app.get("/api/factory-overview/sensors", async (req, res) => {
  ...
  try {
    ...
    if (_sc && Date.now() - _sc.ts < _FACTORY_OVERVIEW_TTL) {
      console.log(
        `⚡ factory sensors cache HIT (date: ${date}, age: ${Math.round((Date.now() - _sc.ts) / 1000)}s)`,
      );
      return res.json(_sc.payload);
    }
    ...
    if (_si) {
      const _r = await _si.catch(() => null);
      if (_r) {
        console.log(`⚡ factory sensors stampede WAIT resolved`);
        return res.json(_r);
      }
    }
    const _sensInflightPromise = new Promise((res, rej) => {
      _sensInflightResolve = res;
      _sensInflightReject = rej;
    });
              ...
              $convert: {
                input: {
                  $trim: {
                    input: {
                      $replaceAll: {
                        input: { $toString: { $ifNull: ["$Humidity", "0"] } },
                        find: "%",
                        replacement: "",
                      },
                    },
    ...
    results.forEach((item) => {
      if (item.factory) {
        ...
        if (item.highestTemp !== null && item.avgHumidity !== null) {
          ...
          wbgt =
            Math.round(
              (0.7 * item.highestTemp +
                0.3 * ((item.avgHumidity / 100) * item.highestTemp)) *
                10,
            ) / 10;
        }
    ...
    const historicalCheck = await collection
      .aggregate([
        {
          $match: {
    ...
    console.log(
      `✅ Sensor data loaded for ${Object.keys(sensorData).length} factories`,
    );
    ...
    _factorySensorsCache.set(_sensCacheKey, {
      ts: Date.now(),
      payload: _sensPayload,
    });
...
app.get("/api/factory-overview/env", async (req, res) => {
  ...
  try {
    ...
    if (_sc && Date.now() - _sc.ts < _FACTORY_OVERVIEW_TTL) {
      console.log(
        `⚡ factory env cache HIT (date: ${date}, age: ${Math.round((Date.now() - _sc.ts) / 1000)}s)`,
      );
      return res.json(_sc.payload);
    }
    ...
    if (_si) {
      const _r = await _si.catch(() => null);
      if (_r) {
        console.log(`⚡ factory env stampede WAIT resolved`);
        return res.json(_r);
      }
    }
    const _envInflightPromise = new Promise((res, rej) => {
      _envInflightResolve = res;
      _envInflightReject = rej;
    });
    ...
    console.log(
      `🌤️  Fetching factory env data for ${factoryNames.length} factories (date: ${date})`,
    );

    // 1. Single MongoDB query for all factory locations
    const masterDb = client.db("Sasaki_Coating_MasterDB");
    const factoryDocs = await masterDb
      .collection("factoryDB")
      .find(
        { 工場: { $in: factoryNames } },
        { projection: { 工場: 1, geotag: 1, coordinates: 1, location: 1 } },
      )
      .toArray();
    ...
    function _defaultEnv() {
      ...
      const temp =
        Math.round(
          (22 +
            Math.sin(((hour - 6) * Math.PI) / 12) * 4 +
            (Math.random() - 0.5) * 2) *
            10,
        ) / 10;
      const hum = Math.round(
        50 + Math.sin((hour * Math.PI) / 12) * 10 + (Math.random() - 0.5) * 10,
      );
      return {
        temperature: Math.max(18, Math.min(26, temp)),
        humidity: Math.max(40, Math.min(60, hum)),
        co2: _simCO2(),
        isDefault: true,
      };
    }
              ...
              env: {
                ...
                apparentTemperature:
                  weatherData.current.apparent_temperature != null
                    ? Math.round(
                        weatherData.current.apparent_temperature * 10,
                      ) / 10
                    : null,
    ...
    factoryNames.forEach((f) => {
      if (!envData[f]) envData[f] = { ..._defaultEnv(), timestamp: Date.now() };
    });

    console.log(
      `✅ Factory env data loaded for ${Object.keys(envData).length} factories`,
    );

    const _envPayload = { success: true, date, data: envData };
    _factoryEnvCache.set(_envCacheKey, {
      ts: Date.now(),
      payload: _envPayload,
    });
...
app.post("/customerCreateUser", async (req, res) => {
  ...
  try {
    ...
    const existingInCustomer = await users.findOne({
      username: normalizedUsername,
    });
    if (existingInCustomer) {
      return res
        .status(400)
        .json({ error: "Username already exists in this customer database" });
    }
    ...
    if (conflictInMaster) {
      return res
        .status(400)
        .json({ error: "Username already exists in a master account" });
    }
    ...
    for (const db of dbs.databases) {
      if (
        ...
        continue;
      const userCol = client.db(db.name).collection("users");
      const existsElsewhere = await userCol.findOne({
        username: normalizedUsername,
      });
      if (existsElsewhere) {
        return res
...
app.post("/customerUpdateRecord", async (req, res) => {
  const { recordId, updateData, dbName, collectionName, role, username } =
    req.body;
...
app.post("/aggregateCustomerDashboardWidgetData", async (req, res) => {
  console.log(
    "🟢 Received POST request to /aggregateCustomerDashboardWidgetData",
  );
  const {
    dbName,
  ...
  if (
    !dbName ||
    !queryConfig ||
    !queryConfig.deviceIdField ||
    !queryConfig.deviceIdValue ||
    !queryConfig.dateField ||
    !queryConfig.dateValue ||
  ...
  try {
    ...
    if (
      queryConfig.additionalFilters &&
      Object.keys(queryConfig.additionalFilters).length > 0
    ) {
    ...
    console.log(
      `Aggregating for widget: ${queryConfig.sourceField}, type: ${queryConfig.summaryType}`,
    );
    ...
    switch (queryConfig.summaryType) {
      ...
      case "max":
        pipeline.push({
          $group: {
            ...
            value: {
              [`$${queryConfig.summaryType}`]: `$${queryConfig.sourceField}`,
            },
          },
        });
      ...
      case "countRecords": // Counts records where the sourceField has a non-null value
        pipeline.push({
          $match: {
            [queryConfig.sourceField]: { $exists: true, $ne: null, $ne: "" },
          },
        });
    ...
    console.log(
      `✅ Aggregation results for ${queryConfig.sourceField}:`,
      results,
    );
    res.json(results);
  } catch (error) {
  ...
  try {
    ...
    if (idsToFetch && idsToFetch.length > 0) {
      try {
        ...
        return res
          .status(400)
          .json({ error: "Invalid ID format in idsToFetch array." });
      }
    } else {
    ...
    console.log(
      "Executing query on submittedDB:",
    ...
    if (getTotalCount) {
      ...
      console.log(
        `✅ Returning ${data.length} records, totalCount: ${totalCount}`,
      );
      return res.json({ data: data, totalCount: totalCount });
    } else {
      console.log(
        `✅ Returning ${data.length} records (no totalCount requested)`,
      );
    ...
    return res
      .status(500)
      .json({ error: "Error fetching submitted logs", details: error.message });
  }
  // finally {
  //     // if (client && client.topology && client.topology.isConnected()) {
  //     //     await client.close();
  ...
  try {
    ...
    if (result.modifiedCount > 0) {
      ...
      if (
        ...
        !Array.isArray(currentDoc.changeHistory)
      ) {
...
app.post("/customerUploadMasterImageWithHistory", async (req, res) => {
  ...
  try {
    ...
    res.json({
      message: "Customer image uploaded successfully with history",
      imageURL: firebaseUrl,
    });
  } catch (error) {
...
app.post("/customerGetMasterDBHistory", async (req, res) => {
  ...
  try {
    ...
    const history = await logs
      .find({ collection: "masterDB" }, { sort: { timestamp: -1 } })
      .toArray();
...
app.post("/customerBulkDeleteWithHistory", async (req, res) => {
  const { recordIds, recordsData, dbName, collectionName, role, username } =
    req.body;
...
app.get("/qr-patterns/hash/:customerType", async (req, res) => {
  try {
    ...
    if (!pattern) {
      return res
        .status(404)
        .json({ error: "No deployed patterns found for customer" });
    }
...
app.post("/qr-learning/learn-patterns", async (req, res) => {
  try {
    ...
    if (customerSamples.length < 3 || internalSamples.length < 3) {
      ...
      return res
        .status(400)
        .json({ error: "Need at least 3 samples of each type" });
    }

    // Analyze patterns using the pattern analysis algorithm
    const analysisResult = await analyzeQRPatterns(
      customerSamples,
      internalSamples,
    );
    ...
    await patterns.replaceOne({ customerType: customerType }, patternRecord, {
      upsert: true,
    });
...
app.post("/qr-learning/extract-product", async (req, res) => {
  try {
    ...
    const learnedPattern = await patterns.findOne({
      customerType: customerType,
    });
    ...
    for (const rule of learnedPattern.extractionRules) {
      if (rule.type === "regex") {
        ...
        if (match && match[rule.captureGroup || 0]) {
          ...
          if (rule.formatting) {
            extractedProduct = applyFormattingServer(
              extractedProduct,
              rule.formatting,
            );
          }
        ...
        console.log(
          `Position extraction: split into ${parts.length} parts, looking for part ${rule.partIndex}`,
        );
        ...
        if (parts.length > rule.partIndex) {
          ...
          console.log(
            `❌ Position extraction failed: only ${parts.length} parts found`,
          );
        }
      }
    }
...
app.post("/qr-learning/test-patterns", async (req, res) => {
  try {
    ...
    const testResults = testSamples.map((sample) => {
      const extractionResult = testExtraction(
        sample.qr,
...
app.delete("/qr-patterns/:customerType", async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      message: `Patterns deleted for ${customerType}`,
    });
  } catch (error) {
...
async function analyzeQRPatterns(customerSamples, internalSamples) {
  ...
  try {
    ...
    const extractionRules = generateExtractionRules(
      customerAnalysis,
      internalAnalysis,
    );
    ...
    console.log(
      "Pattern analysis complete. Extraction rules:",
...
function generateExtractionRules(customerAnalysis, internalAnalysis) {
  ...
  if (structure && structure.partCount === 3) {
    ...
    const productPattern = findProductInThirdPart(
      thirdParts,
      internalAnalysis.products,
    );
...
function findProductAnywhere(customerSamples, internalSamples) {
  ...
  for (
    let i = 0;
    i < customerSamples.length && i < internalSamples.length;
    i++
  ) {
    ...
    const product = internalQR.includes(",")
      ? internalQR.split(",")[0].trim()
      : internalQR.trim();
    ...
    if (customerQR.includes(product)) {
      ...
      const after = customerQR.substring(
        startPos + product.length,
        startPos + product.length + 10,
      );
  ...
  if (
    ...
    productMatches.every((m) => m.hasDirectMatch)
  ) {
...
function calculateConfidence(analysis, rules) {
  ...
  const lengthVariance =
    Math.max(...analysis.lengths) - Math.min(...analysis.lengths);
...
function findCommonStructure(structures) {
  ...
  const isConsistent = structures.every(
    (s) => s.partCount === first.partCount && s.spaceCount === first.spaceCount,
  );
...
function applyFormattingServer(text, formatting) {
  ...
  if (formatting.insert) {
    for (const insert of formatting.insert) {
      result =
        result.substring(0, insert.position) +
        insert.character +
...
async function loadExistingTokens() {
  try {
    ...
    console.log(
      `🔄 Found existing tokens (${Math.round(tokenAge / 1000 / 60)} minutes old)`,
    );
    return tokenData;
  } catch (error) {
...
async function testTokenValidity(phpsessid, csrfToken) {
  ...
  const testUrl =
    "https://sasaki-mfg.gen-cloud.jp/api/received/csv?" +
    new URLSearchParams({
      ...
      offset: "0",
      search_orderby:
        "received_detail_line_no:::false,received_number:::true,id:::false,received_detail_id:::false",
      displayPatternId: "",
    });
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (
        csvText.includes("エラーが発生しました") ||
        csvText.includes("session_error") ||
        csvData.length < 50
      ) {
...
async function getValidTokens() {
  ...
  if (existingTokens) {
    const isValid = await testTokenValidity(
      existingTokens.phpsessid,
      existingTokens.csrfToken,
    );
...
async function downloadCSVWithRetry(apiUrl, tokens, fromDate, toDate) {
  ...
  if (response.ok) {
    ...
    if (
      dataText.includes("エラーが発生しました") ||
      dataText.includes("session_error") ||
      dataText.includes("CSRF") ||
      dataText.includes("expired")
    ) {
      console.log(
        "🔄 Response contains error, tokens may be invalid, retrying with fresh tokens...",
      );
      ...
      if (freshTokens && freshTokens.csrfToken !== "MANUAL_REQUIRED") {
        ...
        const retryResponse = await makeRequest(
          freshTokens.phpsessid,
          freshTokens.csrfToken,
        );

        if (retryResponse.ok) {
          const retryData = await retryResponse.buffer();
          console.log(
            `✅ Retry successful! Downloaded ${retryData.length} bytes`,
          );
    ...
    if (freshTokens && freshTokens.csrfToken !== "MANUAL_REQUIRED") {
      ...
      if (retryResponse.ok) {
        ...
        throw new Error(
          `Both attempts failed. Last error: ${retryResponse.status} ${retryResponse.statusText} - ${retryError.substring(0, 200)}`,
        );
      }
    } else {
      throw new Error("Failed to extract fresh tokens for retry");
...
app.post("/extract-tokens", async (req, res) => {
  ...
  try {
    ...
    if (tokens.csrfToken === "MANUAL_REQUIRED") {
      throw new Error(
        "CSRF token requires manual extraction. Please use the manual mode in the web interface.",
      );
    }
    ...
    const downloadedCSV = await downloadCSVWithRetry(
      apiUrl,
      tokens,
      fromDate,
      toDate,
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename="gen_received_orders_${fromDate || "all"}_to_${toDate || "all"}.csv"`,
    );
    ...
    if (error.message.includes("socket hang up")) {
      userFriendlyMessage =
        "Automated login failed - browser automation blocked";
      instructions =
      ...
      instructions =
        "Your session has expired. Please get fresh tokens using Manual Mode below.";
    }
...
app.post("/api/search-pressdb-exact", async (req, res) => {
  ...
  if (!背番号 || !製造ロット) {
    console.warn(
      "[pressDB-exact] Missing fields — 背番号:",
      背番号,
      "製造ロット:",
      製造ロット,
    );
  ...
  try {
    ...
    console.log(
      "[pressDB-exact] normalizeLotCandidates result:",
      JSON.stringify(lotCandidates),
    );

    const lotValues = Array.from(
      new Set(
        [
          lotCandidates.raw,
    ...
    const query = {
      ...
      $or:
        dateConditions.length > 0
          ? dateConditions
          : [{ Date: 製造ロット }, { 製造ロット: 製造ロット }],
    };
    console.log(
      "[pressDB-exact] DB name: submittedDB | MongoDB query:",
      JSON.stringify(query, null, 2),
    );
    ...
    if (results.length > 0) {
      console.log(
        "[pressDB-exact] First result Date:",
        results[0].Date,
        "背番号:",
        results[0].背番号,
      );
...
app.post("/api/search-pressdb-nearby", async (req, res) => {
  ...
  if (!背番号 || !date) {
    console.warn(
      "[pressDB-nearby] Missing fields — 背番号:",
      背番号,
      "date:",
      date,
    );
  ...
  try {
    ...
    console.log(
      "[pressDB-nearby] normalizeLotCandidates result:",
...
app.get("/api/get-all-recoveries", async (req, res) => {
  ...
  try {
    ...
    const results = await recoveryDB
      .find(query)
      .sort({ createdAt: -1 })
      .toArray();
  ...
  try {
    ...
    if (Array.isArray(pendingImageOps) && pendingImageOps.length > 0) {
      ...
      for (const op of pendingImageOps) {
        ...
        const ext = (op.mimeType || "image/jpeg").includes("png")
          ? ".png"
          : ".jpg";
        const corrN = pendingImageOps.filter(
          (o) =>
            o.type !== "delete" &&
            o.field === op.field &&
            pendingImageOps.indexOf(o) <= pendingImageOps.indexOf(op),
        ).length;
        ...
        if (
          op.type === "replace" &&
          op.index != null &&
          currentImages[op.index] !== undefined
        ) {
    ...
    for (const [field, newVal] of Object.entries(changes)) {
      ...
      if (JSON.stringify(oldVal) !== JSON.stringify(newVal)) {
        ...
        changedFields.push({
          field,
          before: oldVal !== undefined ? oldVal : null,
          after: newVal,
        });
      }
    }
...
app.post("/api/approvals/revert-document", async (req, res) => {
  ...
  try {
    ...
    if (!logEntry)
      return res
        .status(404)
        .json({ success: false, error: "Log entry not found" });
    ...
    const currentBefore = { ...currentDoc };
    delete currentBefore._id;
...
app.get("/api/approvals/recycle-bin", async (req, res) => {
  try {
    await client.connect();
    const recycleColl = client
      .db("recycleBinDB")
      .collection("deleted_approvals");
    const now = new Date();

    // Lazy purge: permanently delete docs that have passed their 1-year expiry
    const expired = await recycleColl
      .find({ expiresAt: { $lte: now } })
      .toArray();
    if (expired.length > 0) {
      const jstNow = new Date(Date.now() + 9 * 60 * 60 * 1000);
      const logColl = client
        .db("Sasaki_Coating_MasterDB")
        .collection("masterDB_Log");
...
app.post("/api/approvals/soft-delete", async (req, res) => {
  const { itemId, collectionName, reason, deletedBy, deletedByUsername } =
    req.body;
...
app.post("/api/approvals/request-delete", async (req, res) => {
  const { itemId, collectionName, reason, requestedBy, requestedByUsername } =
    req.body;
...
app.post("/api/approvals/approve-delete-request", async (req, res) => {
  ...
  try {
    ...
    if (doc.deleteRequestStatus !== "pending_delete") {
      return res
        .status(400)
        .json({ error: "Document is not in pending_delete state" });
    }
      ...
      { _id: docId },
      {
        $push: {
          approvalHistory: {
            ...
            user: approvedBy,
            timestamp: jstNow,
            comment: doc.deleteRequestReason || "",
          },
        },
      },
    );
...
app.post("/api/approvals/reject-delete-request", async (req, res) => {
  ...
  try {
    ...
    if (result.matchedCount === 0) {
      return res
        .status(404)
        .json({ error: "Document not found or not in pending_delete state" });
    }
...
app.post("/api/approvals/restore", async (req, res) => {
  ...
  try {
    ...
    if (!binDoc)
      return res.status(404).json({ error: "Recycle bin document not found" });

    const origColl = client
      .db(binDoc.originalDb)
      .collection(binDoc.originalCollection);
    const jstNow = new Date(Date.now() + 9 * 60 * 60 * 1000);
    ...
    if (!Array.isArray(docToRestore.approvalHistory))
      docToRestore.approvalHistory = [];
    docToRestore.approvalHistory.push({
      action: "ゴミ箱から復元",
      user: restoredBy,
      timestamp: jstNow,
    });
...
app.post("/api/video-manuals/:id/revisions", async (req, res) => {
  try {
    ...
    const revisionName =
      String(req.body?.revisionName || "").trim() ||
      `Revision ${String(revisionNumber).padStart(2, "0")}`;
    const folder =
      String(req.body?.folder || project.folder || "root").trim() || "root";
...
const VM_DEPLOYED_RETENTION_MS =
  VM_DEPLOYED_RETENTION_DAYS * 24 * 60 * 60 * 1000;
...
app.get("/api/video-playlists", async (req, res) => {
  try {
    ...
    const projectCounts = visibleIds.length
      ? await db
          .collection(VM_PROJECTS_COLLECTION)
          .aggregate([
            {
              $match: {
                playlistId: { $in: visibleIds },
                deleted: { $ne: true },
              },
            },
...
app.post("/api/video-playlists", async (req, res) => {
  try {
    ...
    if (!VM_MANAGE_ROLES.has(role)) {
      return res
        .status(403)
        .json({ error: "Insufficient role to create playlists" });
    }
    ...
    const doc = {
      ...
      access: {
        editRoles: Array.isArray(access.editRoles)
          ? access.editRoles
          : ["admin", "課長", "部長", "係長"],
    ...
    const result = await client
      .db(VM_DB)
      .collection(VM_PLAYLISTS_COLLECTION)
      .insertOne(doc);
...
app.delete("/api/video-playlists/:id", async (req, res) => {
  try {
    ...
    const assets = await db
      .collection(VM_ASSETS_COLLECTION)
      .find(
        { playlistId: playlistOid },
    ...
    projects.forEach((project) => {
      if (project.deployedVideoStoragePath)
        storagePaths.add(project.deployedVideoStoragePath);
    ...
    for (const storagePath of storagePaths) {
      try {
        ...
        console.log(
          `[VM Playlist Delete] Deleted from Firebase: ${storagePath}`,
        );
      } catch (err) {
        console.warn(
          `[VM Playlist Delete] Firebase delete failed (${storagePath}):`,
          err.message,
        );
      }
    }
    ...
    if (projectIds.length) {
      await db
        .collection(VM_REVISIONS_COLLECTION)
        .deleteMany({ projectId: { $in: projectIds } });
    }
    await db
      .collection(VM_PROJECTS_COLLECTION)
      .deleteMany({ playlistId: playlistOid });
    await db
      .collection(VM_ASSETS_COLLECTION)
      .deleteMany({ playlistId: playlistOid });
...
app.post("/api/video-playlists/:id/projects", async (req, res) => {
  try {
    ...
    const lastProject = await client
      .db(VM_DB)
      .collection(VM_PROJECTS_COLLECTION)
...
function vmBuildDeployedVideoMetadata(input = {}) {
  ...
  const storagePath =
    String(input.storagePath || input.deployedVideoStoragePath || "").trim() ||
    inferredPath ||
    null;
  ...
  return {
    ...
    storagePath,
    mimeType:
      String(input.mimeType || input.deployedVideoMimeType || "").trim() ||
      "video/mp4",
    fileName:
      String(input.fileName || input.deployedVideoFileName || "").trim() ||
...
async function vmBuildDeployedVideoInventory() {
  ...
  if (missingProjectIds.length) {
    const extraProjects = await db
      .collection(VM_PROJECTS_COLLECTION)
      .find(
        {
          _id: {
            $in: missingProjectIds.map((projectId) => new ObjectId(projectId)),
          },
        },
        { projection: { title: 1 } },
      )
  ...
  revisions.forEach((revision) => {
    ...
    vmAddDeployedVideoReference(referenceMap, storagePath, {
      ...
      projectTitle:
        projectTitleById.get(String(revision.projectId || "")) ||
        "Untitled Project",
  ...
  const inventory = await Promise.all(
    storageFiles
      ...
      .map(async (file) => {
        ...
        const refs = referenceMap.get(file.name) || {
          liveProjects: [],
          revisions: [],
        };
        ...
        return {
          ...
          eligibleForAutoDelete:
            !isLive && !!createdAtMs && ageMs >= VM_DEPLOYED_RETENTION_MS,
          autoDeleteAt: createdAtMs
            ? new Date(createdAtMs + VM_DEPLOYED_RETENTION_MS).toISOString()
            : null,
        };
      }),
  );
...
async function vmDeleteDeployedVideoStoragePath(
  storagePath,
  { deletedBy = "system", allowLive = false } = {},
) {
  ...
  try {
    ...
    const missingFile =
      err?.code === 404 || /No such object/i.test(String(err?.message || ""));
  ...
  console.log(
    `[VM Deployments] Deleted deployed video (${deletedBy}): ${normalizedPath}`,
  );
  return {
    deleted: true,
...
async function vmCleanupExpiredDeployedVideos({ force = false } = {}) {
  ...
  if (
    !force &&
    now - vmDeployedCleanupLastRunAt < VM_DEPLOYED_CLEANUP_INTERVAL_MS
  ) {
  ...
  vmDeployedCleanupPromise = (async () => {
    ...
    for (const file of expiredFiles) {
      try {
        await vmDeleteDeployedVideoStoragePath(file.storagePath, {
          deletedBy: "retention",
          allowLive: false,
        });
        deletedCount += 1;
        ...
        console.warn(
          `[VM Deployments] Retention delete failed for ${file.storagePath}:`,
          err.message,
        );
      }
    }
...
async function vmGetPlaylistActiveAssetReferences(
  db,
  playlistId,
...
async function vmBuildPlaylistAssetUsageIndex(
  db,
  playlistId,
  { excludeProjectId = null } = {},
) {
  ...
  projects.forEach((project) => {
    ...
    const refs = vmCollectProjectAssetReferences(project);
    refs.assetIds.forEach((value) =>
      vmAddProjectUsage(usageIndex.byAssetId, value, projectId),
    );
    refs.urls.forEach((value) =>
      vmAddProjectUsage(usageIndex.byUrl, value, projectId),
    );
    refs.paths.forEach((value) =>
      vmAddProjectUsage(usageIndex.byPath, value, projectId),
    );
  });
...
function vmGetAssetUsageProjectIds(asset, usageIndex) {
  ...
  const inferredStoragePath = vmExtractStoragePath(
...
async function vmPurgeProject(db, project) {
  ...
  for (const url of urlsToPotentiallyDelete) {
    ...
    if (
      activeRefs.urls.has(url) ||
      (storagePath && activeRefs.paths.has(storagePath))
    ) {
    ...
    if (storagePath) {
      try {
        ...
        console.warn(
          `[VM Purge] Firebase delete failed (${storagePath}):`,
          err.message,
        );
      }
    }
...
async function vmAutoPurgeExpired(db) {
  ...
  for (const project of expired) {
    try {
      ...
      console.error("[VM Purge] Error:", e);
    }
  }
}
  ...
  try {
    ...
    const result = docs.map((d) => ({
      ...
      daysRemaining: Math.max(
        0,
        Math.ceil((new Date(d.deletedAt).getTime() + ttlMs - now) / 86400000),
      ),
    }));
  ...
  try {
    ...
    res.json(
      docs.map((doc) => ({
        ...doc,
        isDeployed:
          String(doc._id) === String(project.deployedRevisionId || ""),
      })),
    );
  } catch (err) {
...
app.post("/api/video-projects/:id/deploy", async (req, res) => {
  try {
    ...
    if (!VM_DEPLOY_ROLES.has(role)) {
      return res
        .status(403)
        .json({ error: "Insufficient role to deploy projects" });
    }
    ...
    if (!revision) {
      return res
        .status(404)
        .json({ error: "Revision not found for this project" });
    }
    ...
    if (req.body?.deployedVideo && typeof req.body.deployedVideo === "object") {
      deployedVideoMetadata = vmBuildDeployedVideoMetadata(
        req.body.deployedVideo,
      );
    }
    ...
    const deployedVideoStoragePath =
      String(deployedVideoMetadata.storagePath || "").trim() || null;
    const deployedVideoMimeType =
      String(deployedVideoMetadata.mimeType || "").trim() || "video/mp4";
    const deployedVideoFileName =
      String(deployedVideoMetadata.fileName || "").trim() || null;
...
app.post("/api/video-projects/:id/undeploy", async (req, res) => {
  try {
    ...
    if (!VM_DEPLOY_ROLES.has(role)) {
      return res
        .status(403)
        .json({ error: "Insufficient role to undeploy projects" });
    }
    ...
    void vmCleanupExpiredDeployedVideos().catch((err) => {
      console.warn(
        "[VM Deployments] Post-undeploy cleanup check failed:",
        err.message,
      );
    });
...
app.get("/api/factory/video-manuals", async (req, res) => {
  try {
    ...
    const playlistById = new Map(
      playlists.map((playlist) => [String(playlist._id), playlist]),
    );
    const projects = await db
      .collection(VM_PROJECTS_COLLECTION)
      .aggregate([
...
app.get("/api/factory/video-manuals/projects/:id", async (req, res) => {
  try {
    ...
    if (!project.deployedRevisionId) {
      return res
        .status(404)
        .json({ error: "No deployed revision for this project" });
    }

    const revision = await revisions.findOne({
      _id: new ObjectId(project.deployedRevisionId),
      projectId,
    });
    if (!revision?.snapshot) {
      return res
        .status(404)
        .json({ error: "Deployed revision snapshot not found" });
    }

    const deployedVideoUrl =
      revision.deployedVideoUrl || project.deployedVideoUrl || null;
    const deployedVideoStoragePath =
      revision.deployedVideoStoragePath ||
      project.deployedVideoStoragePath ||
      null;
    const deployedVideoMimeType =
      revision.deployedVideoMimeType || project.deployedVideoMimeType || null;
    const deployedVideoFileName =
      revision.deployedVideoFileName || project.deployedVideoFileName || null;
    ...
    res.json({
      ...
      deployedRevisionId: revision._id,
      deployedRevisionNumber:
        revision.revisionNumber || project.deployedRevisionNumber || null,
      deployedRevisionName:
        revision.revisionName || project.deployedRevisionName || null,
...
app.delete("/api/video-playlists/:id/assets/:assetId", async (req, res) => {
  try {
    ...
    const storagePath =
      asset.storagePath || vmExtractStoragePath(asset.downloadUrl || asset.url);
    if (storagePath) {
      try {
        await admin.storage().bucket().file(storagePath).delete();
        console.log(
          `[VM Assets] Deleted unused asset from Firebase: ${storagePath}`,
        );
      } catch (err) {
        const missingFile =
          err?.code === 404 ||
          /No such object/i.test(String(err?.message || ""));
        if (!missingFile) throw err;
        console.warn(
          `[VM Assets] Asset file already missing in Firebase: ${storagePath}`,
        );
      }
    }
  ...
  try {
    ...
    if (!upstream.ok && upstream.status !== 206) {
      ...
      return res
        ...
        .send(bodyText || "Failed to fetch media");
    }
...
app.post(
  "/api/upload-video-manual",
  express.raw({ type: "*/*", limit: "500mb" }),
  async (req, res) => {
    try {
      const { username, role } = vmGetRequester(req);
      const fileName = vmNormalizeFileName(
        req.headers["x-file-name"] || `video_${Date.now()}.mp4`,
      );
      ...
      const uploadFolder =
        String(req.headers["x-upload-folder"] || "videoManuals").trim() ||
        "videoManuals";
      ...
      if (rawPlaylistId) {
        try {
          ...
          if (!vmCanEdit(playlist, username, role)) {
            return res
              .status(403)
              .json({ error: "No edit access to this playlist" });
          }
        } catch (idErr) {
...
setInterval(() => {
  void vmCleanupExpiredDeployedVideos().catch((err) => {
    console.warn(
      "[VM Deployments] Scheduled retention cleanup failed:",
      err.message,
    );
  });
...
setTimeout(() => {
  void vmCleanupExpiredDeployedVideos({ force: true }).catch((err) => {
    console.warn(
      "[VM Deployments] Initial retention cleanup failed:",
      err.message,
    );
  });
}, 15000);
...
app.listen(port, () => {
  ...
  console.log(
    `🌐 GEN CSV Download available at: http://localhost:${port}/gen-automated`,
  );
});
