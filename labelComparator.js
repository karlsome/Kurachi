...
document.addEventListener("DOMContentLoaded", () => {
  ...
  console.log('🚨 IMMEDIATE DEBUG: Customer loading code is being set up');
  console.log('🚨 availableCustomers.length before timeout:', availableCustomers.length);
  
  // Load customers first, then initialize QR Pattern System
  setTimeout(async () => {
    console.log('🚨 TIMEOUT REACHED! Customer loading starting...');
    console.log('🚀 Starting system initialization...');
    
    try {
      // STEP 1: Load customers from database FIRST
      console.log('📋 STEP 1: Loading customers...');
      await loadCustomersFromDatabase();
      console.log('📋 Customer loading completed, available customers:', availableCustomers.length);
      
      // STEP 2: Initialize QR Pattern System after customers are loaded
      console.log('🔧 STEP 2: Initializing QR Pattern System...');
      await initQRPatternSystem(serverURL);
      console.log('QR Pattern System initialized successfully');
      
      // STEP 3: Sync patterns for all loaded customers
      console.log('🔄 STEP 3: Syncing patterns for customers...');
      const customerCodes = availableCustomers.map(c => c.customerCode);
      console.log('🎯 Syncing patterns for:', customerCodes);
      
      for (const customer of customerCodes) {
        try {
          await qrPatternSync.syncCustomerPattern(customer);
          console.log(`✅ Synced patterns for ${customer}`);
        } catch (error) {
          console.warn(`⚠️ Failed to sync patterns for ${customer}:`, error);
        }
      }
  ...
  setTimeout(() => {
    ...
    // IMPORTANT: Also call customer loading here as backup
    // This ensures customers are loaded even if the main initialization fails
    if (availableCustomers.length === 0 && !isLoadingCustomers) {
      console.log('🔄 Backup customer loading triggered...');
      loadCustomersFromDatabase().catch(error => {
        console.error('Backup customer loading failed:', error);
      });
    }
  ...
  async function loadCustomersFromDatabase() {
    console.log('🔄 loadCustomersFromDatabase() called');
    
    if (isLoadingCustomers) {
      console.log('⚠️ Already loading customers, skipping...');
      return;
    }
    ...
    try {
      console.log(`🌐 Fetching from: ${serverURL}/api/labelComparator/customers`);
      ...
      if (response.ok && result.success) {
        ...
        console.log(`✅ Loaded ${availableCustomers.length} customers:`, availableCustomers);
        populateCustomerDropdown();
        console.log(`✅ Loaded ${availableCustomers.length} customers from database`);
      } else {
      ...
      console.log('🔄 Falling back to hardcoded customers...');
      availableCustomers = [
  ...
  function populateCustomerDropdown() {
    console.log('🔧 populateCustomerDropdown() called with', availableCustomers.length, 'customers');
    ...
    if (!menu || !selectedText || !hiddenInput) {
    ...
    availableCustomers.forEach((customer, index) => {
      console.log(`➕ Adding customer ${index + 1}:`, customer);
      
      const option = document.createElement('div');
      option.className = 'custom-dropdown-option' + (index === 0 ? ' selected' : '');
      option.setAttribute('data-value', customer.customerCode);
      ...
      option.addEventListener('click', function() {
        ...
        console.log('🎯 Customer selected:', value, text);
      });
    });
    
    // Set first customer as default if available
    if (availableCustomers.length > 0) {
      const firstCustomer = availableCustomers[0];
      console.log('🎯 Setting default customer:', firstCustomer);
      selectedText.textContent = firstCustomer.displayName;
      hiddenInput.value = firstCustomer.customerCode;
      
      // Trigger change event to initialize form data
      setTimeout(() => {
        const event = new Event('change', { bubbles: true });
        hiddenInput.dispatchEvent(event);
        console.log('🔄 Triggered change event for default customer');
      }, 100);
    }
  ...
  // Make customer loading function globally available for debugging
  window.loadCustomers = loadCustomersFromDatabase;
  window.getAvailableCustomers = () => availableCustomers;
});
